...
import autanac
from autanac.modules import get_modules
from autanac.modules.module import run_unit_tests, get_test_modules
from autanac.modules.registry import Registry
from autanac.release import nt_service_name
from autanac.tools import config
from autanac.tools import stripped_sys_argv, dumpstacks, log_ormcache_stats
...
class RequestHandler(werkzeug.serving.WSGIRequestHandler):
    def setup(self):
        ...
        me.name = 'autanac.service.http.request.%s' % (me.ident,)
...
class ThreadedWSGIServerReloadable(LoggingBaseWSGIServerMixIn, werkzeug.serving.ThreadedWSGIServer):
    ...
    def __init__(self, host, port, app):
        # The autanac_MAX_HTTP_THREADS environment variable allows to limit the amount of concurrent
        ...
        self.max_http_threads = os.environ.get("autanac_MAX_HTTP_THREADS")
    ...
    # is included in all Python 3 releases installed on all operating systems supported by autanac.
    ...
    def handle_file(self, path):
        if path.endswith('.py') and not os.path.basename(path).startswith('.~'):
            try:
                ...
                if not getattr(autanac, 'phoenix', False):
...
class FSWatcherWatchdog(FSWatcherBase):
    def __init__(self):
        ...
        for path in autanac.addons.__path__:
...
class FSWatcherInotify(FSWatcherBase):
    ...
    def start(self):
        self.started = True
        self.thread = threading.Thread(target=self.run, name="autanac.service.autoreload.watcher")
...
class ThreadedServer(CommonServer):
    ...
    def cron_thread(self, number):
        from autanac.addons.base.models.ir_cron import ir_cron
        while True:
            time.sleep(SLEEP_INTERVAL + number)     # Steve Reich timing style
            registries = autanac.modules.registry.Registry.registries
    ...
    def cron_spawn(self):
        ...
        for i in range(autanac.tools.config['max_cron_threads']):
            def target():
                self.cron_thread(i)
            t = threading.Thread(target=target, name="autanac.service.cron.cron%d" % i)
            t.setDaemon(True)
            t.type = 'cron'
            t.start()
    ...
    def http_spawn(self):
        t = threading.Thread(target=self.http_thread, name="autanac.service.httpd")
        t.setDaemon(True)
        t.start()
    ...
    def stop(self):
        ...
        """
        if getattr(autanac, 'phoenix', None):
...
class Worker(object):
    ...
    def setproctitle(self, title=""):
        setproctitle('autanac: %s %s %s' % (self.__class__.__name__, self.pid, title))
    ...
    def run(self):
        try:
            ...
            _logger.info("Worker (%s) exiting. request_count: %s, registry count: %s.",
                         ...
                         len(autanac.modules.registry.Registry.registries))
            self.stop()
        except Exception:
...
class WorkerHTTP(Worker):
    ...
    def __init__(self, multi):
        ...
        # The autanac_HTTP_SOCKET_TIMEOUT environment variable allows to control socket timeout for
        ...
        sock_timeout = os.environ.get("autanac_HTTP_SOCKET_TIMEOUT")
...
class WorkerCron(Worker):
    ...
    def _db_list(self):
        if config['db_name']:
            ...
            db_names = autanac.service.db.list_dbs(True)
        return db_names

    def process_work(self):
        rpc_request = logging.getLogger('autanac.netsvc.rpc.request')
        ...
        if len(db_names):
            ...
            from autanac.addons import base
...
def load_server_wide_modules():
    server_wide_modules = {'base', 'web'} | set(autanac.conf.server_wide_modules)
    for m in server_wide_modules:
        try:
            autanac.modules.module.load_openerp_module(m)
        except Exception:
            msg = ''
            if m == 'web':
                msg = """
...
def _reexec(updated_modules=None):
    ...
    if autanac.tools.osutil.is_running_as_nt_service():
        ...
        for mod in [m for m in get_modules() if '/%s/' % m in test_file]:
            for mod_mod in get_test_modules(mod):
                ...
                if test_path == mod_path:
                    ...
                    result = autanac.modules.module.autanacTestRunner().run(suite)
        ...
        try:
            ...
            if config['test_enable']:
                ...
                _logger.info("All post-tested in %.2fs, %s queries",
                             time.time() - t0, autanac.sql_db.sql_counter - t0_sql)
...
def start(preload=None, stop=False):
    """ Start the autanac http server and cron processor.
    """
    global server

    load_server_wide_modules()
    autanac.service.wsgi_server._patch_xmlrpc_marshaller()

    if autanac.evented:
        server = GeventServer(autanac.service.wsgi_server.application)
    elif config['workers']:
        if config['test_enable'] or config['test_file']:
            _logger.warning("Unit testing in workers mode could fail; use --workers 0.")

        server = PreforkServer(autanac.service.wsgi_server.application)
        ...
        if platform.system() == "Linux" and sys.maxsize > 2**32 and "MALLOC_ARENA_MAX" not in os.environ:
            ...
            # which autanac is based upon in order to limit the memory usage for threaded workers.
        ...
        server = ThreadedServer(autanac.service.wsgi_server.application)

    watcher = None
    if 'reload' in config['dev_mode'] and not autanac.evented:
        if inotify:
    ...
    if getattr(autanac, 'phoenix', False):
        _reexec()
