...
import autanac
from autanac.exceptions import UserError, ValidationError, QWebException
from autanac.models import check_method_name
from autanac.tools.translate import translate, translate_sql_constraint
...
def dispatch(method, params):
    ...
    registry = autanac.registry(db).check_signaling()
    fn = globals()[method]
...
def check(f):
    ...
    def wrapper(___dbname, *args, **kwargs):
        ...
        def tr(src, ttype):
            ...
            with closing(autanac.sql_db.db_connect(dbname).cursor()) as cr:
        ...
        while True:
            try:
                if autanac.registry(dbname)._init and not autanac.tools.config['test_enable']:
                    raise autanac.exceptions.Warning('Currently, this database is not fully loaded and can not be used.')
...
def execute_cr(cr, uid, obj, method, *args, **kw):
    autanac.api.Environment.reset()  # clean cache etc if we retry the same transaction
    recs = autanac.api.Environment(cr, uid, {}).get(obj)
    if recs is None:
        raise UserError(_("Object %s doesn't exist") % obj)
    result = autanac.api.call_kw(recs, method, args, kw)
...
def execute(db, uid, obj, method, *args, **kw):
    ...
    with autanac.registry(db).cursor() as cr:
