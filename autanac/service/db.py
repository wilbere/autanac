...
import autanac.sql_db
import autanac.tools
from autanac.sql_db import db_connect
from autanac.release import version_info
...
def check_super(passwd):
    if passwd and autanac.tools.config.verify_admin_password(passwd):
        return True
    raise autanac.exceptions.AccessDenied()

# This should be moved to autanac.modules.db, along side initialize().
def _initialize_db(id, db_name, demo, lang, user_password, login='admin', country_code=None, phone=None):
    try:
        db = autanac.sql_db.db_connect(db_name)
        with closing(db.cursor()) as cr:
            # TODO this should be removed as it is done by Registry.new().
            autanac.modules.db.initialize(cr)
            autanac.tools.config['load_language'] = lang
            cr.commit()

        registry = autanac.modules.registry.Registry.new(db_name, demo, None, update_module=True)

        with closing(db.cursor()) as cr:
            env = autanac.api.Environment(cr, SUPERUSER_ID, {})
            ...
            if login:
                values['login'] = login
                emails = autanac.tools.email_split(login)
                if emails:
...
def _create_empty_database(name):
    db = autanac.sql_db.db_connect('postgres')
    with closing(db.cursor()) as cr:
        chosen_template = autanac.tools.config['db_template']
...
def exp_duplicate_database(db_original_name, db_name):
    ...
    autanac.sql_db.close_db(db_original_name)
    db = autanac.sql_db.db_connect('postgres')
    with closing(db.cursor()) as cr:
        cr.autocommit(True)     # avoid transaction block
    ...
    registry = autanac.modules.registry.Registry.new(db_name)
    with registry.cursor() as cr:
        # if it's a copy of a database, force generation of a new dbuuid
        env = autanac.api.Environment(cr, SUPERUSER_ID, {})
        env['ir.config_parameter'].init(force=True)

    from_fs = autanac.tools.config.filestore(db_original_name)
    to_fs = autanac.tools.config.filestore(db_name)
...
def exp_drop(db_name):
    if db_name not in list_dbs(True):
        return False
    autanac.modules.registry.Registry.delete(db_name)
    autanac.sql_db.close_db(db_name)
    ...
    fs = autanac.tools.config.filestore(db_name)
...
def dump_db_manifest(cr):
    ...
    manifest = {
        'autanac_dump': '1',
        'db_name': cr.dbname,
        'version': autanac.release.version,
        'version_info': autanac.release.version_info,
        'major_version': autanac.release.major_version,
...
def dump_db(db_name, stream, backup_format='zip'):
    ...
    if backup_format == 'zip':
        with autanac.tools.osutil.tempdir() as dump_dir:
            filestore = autanac.tools.config.filestore(db_name)
            ...
            autanac.tools.exec_pg_command(*cmd)
            if stream:
                autanac.tools.osutil.zip_dir(dump_dir, stream, include_dir=False, fnct_sort=lambda file_name: file_name != 'dump.sql')
            else:
                t=tempfile.TemporaryFile()
                autanac.tools.osutil.zip_dir(dump_dir, t, include_dir=False, fnct_sort=lambda file_name: file_name != 'dump.sql')
                t.seek(0)
                return t
    else:
        cmd.insert(-1, '--format=c')
        stdin, stdout = autanac.tools.exec_pg_command_pipe(*cmd)
        if stream:
...
def restore_db(db, dump_file, copy=False):
    ...
    with autanac.tools.osutil.tempdir() as dump_dir:
        ...
        if autanac.tools.exec_pg_command(pg_cmd, *pg_args):
            raise Exception("Couldn't restore database")

        registry = autanac.modules.registry.Registry.new(db)
        with registry.cursor() as cr:
            env = autanac.api.Environment(cr, SUPERUSER_ID, {})
            if copy:
                # if it's a copy of a database, force generation of a new dbuuid
            ...
            if autanac.tools.config['unaccent']:
                try:
...
def exp_rename(old_name, new_name):
    autanac.modules.registry.Registry.delete(old_name)
    autanac.sql_db.close_db(old_name)
    ...
    old_fs = autanac.tools.config.filestore(old_name)
    new_fs = autanac.tools.config.filestore(new_name)
...
def exp_change_admin_password(new_password):
    autanac.tools.config.set_admin_password(new_password)
    autanac.tools.config.save()
    return True
...
def exp_migrate_databases(databases):
    for db in databases:
        ...
        autanac.tools.config['update']['base'] = True
        autanac.modules.registry.Registry.new(db, force_demo=False, update_module=True)
    return True
...
@autanac.tools.mute_logger('autanac.sql_db')
...
def list_dbs(force=False):
    if not autanac.tools.config['list_db'] and not force:
        raise autanac.exceptions.AccessDenied()

    if not autanac.tools.config['dbfilter'] and autanac.tools.config['db_name']:
        # In case --db-filter is not provided and --database is passed, autanac will not
        ...
        res = sorted(db.strip() for db in autanac.tools.config['db_name'].split(','))
        return res
    ...
    with closing(db.cursor()) as cr:
        try:
            ...
            res = [autanac.tools.ustr(name) for (name,) in cr.fetchall()]
        except Exception:
...
def list_db_incompatible(databases):
    """"Check a list of databases if they are compatible with this version of autanac
    ...
    for database_name in databases:
        with closing(db_connect(database_name).cursor()) as cr:
            if autanac.tools.table_exists(cr, 'ir_module_module'):
    ...
    for database_name in incompatible_databases:
        # release connection
        autanac.sql_db.close_db(database_name)
...
def exp_list_lang():
    return autanac.tools.scan_languages()

def exp_list_countries():
    list_countries = []
    root = ET.parse(os.path.join(autanac.tools.config['root_path'], 'addons/base/data/res_country_data.xml')).getroot()
...
def exp_server_version():
    ...
    """
    return autanac.release.version
