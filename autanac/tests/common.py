# -*- coding: utf-8 -*-
"""
The module :mod:`autanac.tests.common` provides unittest test cases and a few
...
from autanac.models import BaseModel
from autanac.osv.expression import normalize_domain, TRUE_LEAF, FALSE_LEAF
from autanac.tools import float_compare, single_email_re
from autanac.tools.misc import find_in_path
...
import autanac
import pprint
from autanac import api
from autanac.service import security



_logger = logging.getLogger(__name__)

# The autanac library is supposed already configured.
ADDONS_PATH = autanac.tools.config['addons_path']
HOST = '127.0.0.1'
PORT = autanac.tools.config['http_port']
# Useless constant, tests are aware of the content of demo data
ADMIN_USER_ID = autanac.SUPERUSER_ID


def get_db_name():
    db = autanac.tools.config['db_name']
...
def new_test_user(env, login='', groups='base.group_user', context=None, **kwargs):
     ...
     * email: it is either the login (if it is a valid email) or a generated
       ...
       to email being required for most autanac operations;
    """
    if not login:
...
class MetaCase(type):
    ...
    def __init__(cls, name, bases, attrs):
        ...
        if cls.__module__.startswith('autanac.addons.'):
...
class BaseCase(TreeCase, MetaCase('DummyCase', (object,), {})):
    ...
    longMessage = True      # more verbose error message by default: https://www.autanac.com/r/Vmh
    ...
    def browse_ref(self, xid):
        ...
        :param xid: fully-qualified :term:`external identifier`, in the form
                    :samp:`{module}.{identifier}`
        :raise: ValueError if not found
        :returns: :class:`~autanac.models.BaseModel`
        """
        ...
        if self.warm:
            ...
            with self.subTest(), patch('random.random', lambda: 1):
                ...
                if count != expected:
                    ...
                    if "/autanac/addons/" in filename:
                        filename = filename.rsplit("/autanac/addons/", 1)[1]
                    if count > expected:
...
class TransactionCase(BaseCase):
    ...
    def setUp(self):
        ...
        self.registry = autanac.registry(get_db_name())
        ...
        #: :class:`~autanac.api.Environment` for the current test case
        self.env = api.Environment(self.cr, autanac.SUPERUSER_ID, {})
...
class SingleTransactionCase(BaseCase):
    ...
    def setUpClass(cls):
        ...
        cls.registry = autanac.registry(get_db_name())
        cls.cr = cls.registry.cursor()
        cls.env = api.Environment(cls.cr, autanac.SUPERUSER_ID, {})
...
class ChromeBrowser():
    ...
    def __init__(self, logger, window_size, test_class):
        ...
        self.request_id = 0
        self.user_data_dir = tempfile.mkdtemp(suffix='_chrome_autanac')
        self.chrome_pid = None

        otc = autanac.tools.config
    ...
    def _from_remoteobject(self, arg):
        ...
        # of the way autanac widgets are created they all appear as Class(...)
...
class HttpCase(TransactionCase):
    ...
    def setUp(self):
        ...
        self.session = autanac.http.root.session_store.new()
        self.session_id = self.session.sid
        self.session.db = get_db_name()
        autanac.http.root.session_store.save(self.session)
    ...
    def _wait_remaining_requests(self, timeout=10):
        ...
        def get_http_request_threads():
            return [t for t in threading.enumerate() if t.name.startswith('autanac.service.http.request.')]
    ...
    def authenticate(self, user, password):
        ...
        if user is None:
            if self.session:
                autanac.http.root.session_store.delete(self.session)
        ...
        autanac.http.root.session_store.save(session)
        if self.browser:
    ...
    def start_tour(self, url_path, tour_name, step_delay=None, **kwargs):
        ...
        code = kwargs.pop('code', "autanac.startTour('%s'%s)" % (tour_name, step_delay))
        ready = kwargs.pop('ready', "autanac.__DEBUG__.services['web_tour.tour'].tours.%s.ready" % tour_name)
...
class Form(object):
    ...
    * properly handle defaults & onchanges around x2many fields
    ...
    :class:`~autanac.fields.Many2one` fields assign a singleton recordset::
    ...
    For :class:`~autanac.fields.Many2many` fields, the field itself is a
    :class:`~autanac.tests.common.M2MProxy` and can be altered by adding or
    removing records::
    ...
    Finally :class:`~autanac.fields.One2many` are reified as
    :class:`~autanac.tests.common.O2MProxy`.

    Because the :class:`~autanac.fields.One2many` only exists through its
    parent, it is manipulated more directly by creating "sub-forms"
    with the :meth:`~autanac.tests.common.O2MProxy.new` and
    :meth:`~autanac.tests.common.O2MProxy.edit` methods. These would
    ...
    :type recordp: autanac.models.Model
    ...
    :type view: int | str | autanac.model.Model
    ...
    def _init_from_defaults(self, model):
        ...
        def cleanup(k, v):
            if fields[k]['type'] == 'one2many':
                ...
                return autanac.fields.Datetime.to_string(v)
...
class O2MProxy(X2MProxy):
    ...
    def new(self):
        ...
        :class:`~autanac.fields.One2many` record, properly initialised.
    ...
    def edit(self, index):
        ...
        :class:`~autanac.fields.One2many` record.
...
def record_to_values(fields, record):
    ...
    for f, descr in fields.items():
        ...
        if descr['type'] == 'many2one':
            ...
            v = autanac.fields.Datetime.to_string(v)
        elif descr['type'] == 'date' and isinstance(v, date):
            v = autanac.fields.Date.to_string(v)
        r[f] = v
    return r
...
def _cleanup_from_default(type_, value):
    ...
    if type_ == 'one2many':
        ...
        return autanac.fields.Datetime.to_string(value)
    elif type_ == 'date' and isinstance(value, date):
        return autanac.fields.Date.to_string(value)
    return value
...
def tagged(*tags):
    ...
    By default, all Test classes from autanac.tests.common have a test_tags
