...
from autanac.loglevels import get_encodings, ustr, exception_to_unicode     # noqa
...
def exec_pg_environ():
    ...
    """
    env = os.environ.copy()
    if autanac.tools.config['db_host']:
        env['PGHOST'] = autanac.tools.config['db_host']
    if autanac.tools.config['db_port']:
        env['PGPORT'] = str(autanac.tools.config['db_port'])
    if autanac.tools.config['db_user']:
        env['PGUSER'] = autanac.tools.config['db_user']
    if autanac.tools.config['db_password']:
        env['PGPASSWORD'] = autanac.tools.config['db_password']
    return env
...
def file_open(name, mode="r", subdir='addons', pathinfo=False):
    ...
    """
    import autanac.modules as addons
    adps = autanac.addons.__path__
...
def _fileopen(path, mode, basedir, pathinfo, basename=None):
    ...
    import autanac.modules as addons
    paths = autanac.addons.__path__ + [config['root_path']]
...
def scan_languages():
    ...
    :rtype: [(str, unicode)]
    """
    csvpath = autanac.modules.module.get_resource_path('base', 'data', 'res.lang.csv')
    try:
...
class mute_logger(object):
    ...
    Can be used as context manager or decorator.
        ...
        @mute_logger('autanac.plic.ploc')
        def do_stuff():
            blahblah()

        with mute_logger('autanac.foo.bar'):
            do_suff()
...
def format_date(env, value, lang_code=False, date_format=False):
    ...
    if isinstance(value, str):
        ...
        if len(value) > DATE_LENGTH:
            ...
            value = autanac.fields.Datetime.from_string(value)
            value = autanac.fields.Datetime.context_timestamp(env['res.lang'], value)
        else:
            value = autanac.fields.Datetime.from_string(value)
...
def format_datetime(env, value, tz=False, dt_format='medium', lang_code=False):
    ...
    if not value:
        return ''
    if isinstance(value, str):
        timestamp = autanac.fields.Datetime.from_string(value)
    else:
        timestamp = value
