...
import autanac
import autanac.tools as tools
import autanac.release as release
from autanac import SUPERUSER_ID, api
...
# ad_paths is a deprecated alias, please use autanac.addons.__path__
@tools.lazy
def ad_paths():
    warnings.warn(
        '"autanac.modules.module.ad_paths" is a deprecated proxy to '
        '"autanac.addons.__path__".', DeprecationWarning, stacklevel=2)
    return autanac.addons.__path__
...
class AddonsHook(object):
    ...
    def find_module(self, name, path=None):
        if name.startswith('openerp.addons.') and name.count('.') == 2:
            warnings.warn(
                '"openerp.addons" is a deprecated alias to "autanac.addons".',
    ...
    def load_module(self, name):
        ...
        autanac_name = re.sub(r'^openerp.addons.(\w+)$', r'autanac.addons.\g<1>', name)

        autanac_module = sys.modules.get(autanac_name)
        if not autanac_module:
            autanac_module = importlib.import_module(autanac_name)

        sys.modules[name] = autanac_module

        return autanac_module
...
class autanacHook(object):
    """ Makes autanac package also available as openerp """
    ...
    def find_module(self, name, path=None):
        ...
        if re.match(r'^openerp\b', name):
            warnings.warn(
                'openerp is a deprecated alias to autanac.',
                DeprecationWarning, stacklevel=2)
            return self
    ...
    def load_module(self, name):
        ...
        canonical = re.sub(r'^openerp(.*)', r'autanac\g<1>', name)
...
def initialize_sys_path():
    ...
    ``import autanac.addons.crm``) works even if the addons are not in the
    PYTHONPATH.
    """
    # hook autanac.addons on data dir
    dd = os.path.normcase(tools.config.addons_data_dir)
    if os.access(dd, os.R_OK) and dd not in autanac.addons.__path__:
        autanac.addons.__path__.append(dd)

    # hook autanac.addons on addons paths
    for ad in tools.config['addons_path'].split(','):
        ad = os.path.normcase(os.path.abspath(tools.ustr(ad.strip())))
        if ad not in autanac.addons.__path__:
            autanac.addons.__path__.append(ad)

    # hook autanac.addons on base module path
    base_path = os.path.normcase(os.path.abspath(os.path.join(os.path.dirname(os.path.dirname(__file__)), 'addons')))
    if base_path not in autanac.addons.__path__ and os.path.isdir(base_path):
        autanac.addons.__path__.append(base_path)

    # hook autanac.upgrade on upgrade-path
    from autanac import upgrade
    ...
    # create decrecated module alias from autanac.addons.base.maintenance.migrations to autanac.upgrade
    spec = importlib.machinery.ModuleSpec("autanac.addons.base.maintenance", None, is_package=True)
    maintenance_pkg = importlib.util.module_from_spec(spec)
    maintenance_pkg.migrations = upgrade
    sys.modules["autanac.addons.base.maintenance"] = maintenance_pkg
    sys.modules["autanac.addons.base.maintenance.migrations"] = upgrade

    if not getattr(initialize_sys_path, 'called', False): # only initialize once
        sys.meta_path.insert(0, autanacHook())
...
def get_module_path(module, downloaded=False, display_warning=True):
    ...
    """
    initialize_sys_path()
    for adp in autanac.addons.__path__:
...
def get_module_filetree(module, dir='.'):
    ...
    files = autanac.tools.osutil.listdir(path, True)
...
def get_resource_from_path(path):
    ...
    """
    resource = False
    for adpath in autanac.addons.__path__:
...
def load_information_from_description_file(module, mod_path=None):
    ...
    if manifest_file:
        ...
        info = {
            'application': False,
            'author': 'Distribuidora Autana Cars',
            'auto_install': False,
...
def load_openerp_module(module_name):
    ...
    initialize_sys_path()
    try:
        __import__('autanac.addons.' + module_name)
        ...
        if info['post_load']:
            getattr(sys.modules['autanac.addons.' + module_name], info['post_load'])()
...
def get_modules():
    ...
    plist = []
    initialize_sys_path()
    for ad in autanac.addons.__path__:
...
def get_test_modules(module):
    ...
    results = _get_tests_modules('autanac.addons', module)

    try:
        importlib.import_module('autanac.upgrade.%s' % module)
    except ImportError:
        pass
    else:
        results += _get_tests_modules('autanac.upgrade', module)
...
class autanacTestResult(unittest.result.TestResult):
    """
    ...
    def getErrorCallerInfo(self, error, test):
        ...
        # only test case should be executed in autanac, this is only a safe guard
...
def run_unit_tests(module_name, position='at_install'):
    ...
    """
    global current_test
    from autanac.tests.common import TagsSelector # Avoid import loop
    ...
    for m in mods:
        ...
        if suite.countTestCases():
            t0 = time.time()
            t0_sql = autanac.sql_db.sql_counter
            _logger.info('%s running tests.', m.__name__)
            result = autanacTestRunner().run(suite)
            if time.time() - t0 > 5:
                _logger.log(25, "%s tested in %.2fs, %s queries", m.__name__, time.time() - t0, autanac.sql_db.sql_counter - t0_sql)
