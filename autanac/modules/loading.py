...
import autanac
import autanac.modules.db
import autanac.modules.graph
import autanac.modules.migration
import autanac.modules.registry
import autanac.tools as tools

from autanac import api, SUPERUSER_ID
from autanac.modules.module import adapt_version, initialize_sys_path, load_openerp_module

_logger = logging.getLogger(__name__)
_test_logger = logging.getLogger('autanac.tests')
...
def force_demo(cr):
    ...
    """
    graph = autanac.modules.graph.Graph()
...
def load_module_graph(cr, graph, status=None, perform_checks=True,
    ...
    def load_test(idref, mode):
        ...
        try:
            ...
            autanac.registry(cr.dbname).clear_caches()
    ...
    processed_modules = []
    loaded_modules = []
    registry = autanac.registry(cr.dbname)
    migrations = autanac.modules.migration.MigrationManager(cr, graph)
    ...
    for index, package in enumerate(graph, 1):
        ...
        if new_install:
            py_module = sys.modules['autanac.addons.%s' % (module_name,)]
        ...
        if needs_update:
            ...
            overwrite = autanac.tools.config["overwrite_existing_translations"]
            ...
            if tools.config.options['test_enable']:
                ...
                report.record_result(autanac.modules.module.run_unit_tests(module_name))
    ...
    _logger.log(25, "%s modules loaded in %.2fs, %s queries", len(graph), time.time() - t0, autanac.sql_db.sql_counter - t0_sql)
...
def load_modules(db, force_demo=False, status=None, update_module=False):
    ...
    with db.cursor() as cr:
        if not autanac.modules.db.is_initialized(cr):
        ...
        # autanac.modules.registry.Registry.new().
        registry = autanac.registry(cr.dbname)
        ...
        if update_module:
            ...
            if modules_to_remove:
                ...
                for pkg in pkgs:
                    ...
                    if uninstall_hook:
                        py_module = sys.modules['autanac.addons.%s' % (pkg.name,)]
                ...
                registry = autanac.modules.registry.Registry.new(
