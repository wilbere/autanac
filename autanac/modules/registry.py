...
from autanac.tools import (assertion_report, config, existing_tables, ignore,
                        lazy_classproperty, lazy_property, OrderedSet)
from autanac.tools.lru import LRU

_logger = logging.getLogger(__name__)
_schema = logging.getLogger('autanac.schema')
...
class Registry(Mapping):
    ...
    def __new__(cls, db_name):
        ...
        with cls._lock:
            try:
                ...
                # autanac.service.wsgi_server.application
        ...
        with cls._lock:
            with autanac.api.Environment.manage():
                ...
                try:
                    ...
                    try:
                        autanac.modules.load_modules(registry._db, force_demo, status, update_module)
                    except Exception:
                        autanac.modules.reset_modules_state(db_name)
                        raise
                except Exception:
    ...
    def init(self, db_name):
        ...
        self._db = autanac.sql_db.db_connect(db_name)
        ...
        with closing(self.cursor()) as cr:
            has_unaccent = autanac.modules.db.has_unaccent(cr)
            if autanac.tools.config['unaccent'] and not has_unaccent:
                _logger.warning("The option --unaccent was given but no unaccent() function was found in database.")
            self.has_unaccent = autanac.tools.config['unaccent'] and has_unaccent
    ...
    def init_models(self, cr, model_names, context, install=True):
        ...
        env = autanac.api.Environment(cr, SUPERUSER_ID, context)
