...
class Shell(Command):
    """Start autanac in an interactive shell"""
    supported_shells = ['ipython', 'ptpython', 'bpython', 'python']

    def init(self, args):
        config.parse_config(args)
        autanac.cli.server.report_configuration()
        autanac.service.server.start(preload=[], stop=True)
        ...
        with autanac.api.Environment.manage():
            if dbname:
                ...
                with registry.cursor() as cr:
                    ...
                    ctx = autanac.api.Environment(cr, uid, {})['res.users'].context_get()
                    env = autanac.api.Environment(cr, uid, ctx)
                    local_vars['env'] = env
