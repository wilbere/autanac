...
(c) 2003-TODAY, Fabien Pinckaers - Autana Cars
"""
...
__author__ = autanac.release.author
__version__ = autanac.release.version

# Also use the `autanac` logger for the main script.
_logger = logging.getLogger('autanac')
...
def report_configuration():
    ...
    """
    config = autanac.tools.config
    _logger.info("autanac version %s", __version__)
    if os.path.isfile(config.rcfile):
        _logger.info("Using configuration file at " + config.rcfile)
    _logger.info('addons paths: %s', autanac.addons.__path__)
...
def setup_pid_file():
    ...
    """
    config = autanac.tools.config
    if not autanac.evented and config['pidfile']:
        pid = os.getpid()
...
def export_translation():
    ...
    with open(config["translate_out"], "wb") as buf:
        registry = autanac.modules.registry.Registry.new(dbname)
        with autanac.api.Environment.manage():
            with registry.cursor() as cr:
                autanac.tools.trans_export(config["language"],
...
def import_translation():
    ...
    with autanac.api.Environment.manage():
        with registry.cursor() as cr:
            autanac.tools.trans_load(
                cr, config["translate_in"], config["language"], context=context,
            )

def main(args):
    check_root_user()
    autanac.tools.config.parse_config(args)
    check_postgres_user()
    ...
    if config['db_name']:
        ...
        for db_name in preload:
            try:
                autanac.service.db._create_empty_database(db_name)
                ...
                if err.pgcode == errorcodes.INSUFFICIENT_PRIVILEGE:
                    ...
                    raise err
            except autanac.service.db.DatabaseExists:
                pass
    ...
    if config['workers']:
        autanac.multi_process = True

    stop = config["stop_after_init"]

    setup_pid_file()
    rc = autanac.service.server.start(preload=preload, stop=stop)
    sys.exit(rc)

class Server(Command):
    """Start the autanac server (default command)"""
    def run(self, args):
        main(args)
