...
from autanac.tools import cloc, config
from . import Command

class Cloc(Command):
    """\
    autanac cloc is a tool to count the number of relevant lines written in
    ...
    It has two modes of operation, either by providing a path:
        ...
        autanac-bin cloc -p module_path

    Or by providing the name of a database:

        autanac-bin cloc --addons-path=dirs -d database
