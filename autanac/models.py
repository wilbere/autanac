...
class MetaModel(api.Meta):
    ...
    def _get_addon_name(self, full_name):
        # The (OpenERP) module name can be in the ``autanac.addons`` namespace
        # or not. For instance, module ``sale`` can be imported as
        # ``autanac.addons.sale`` (the right way) or ``sale`` (for backward
        # compatibility).
        module_parts = full_name.split('.')
        if len(module_parts) > 2 and module_parts[:2] == ['autanac', 'addons']:
...
class BaseModel(MetaModel('DummyModel', (object,), {'_register': False})):
    """Base class for autanac models.

    autanac models are created by inheriting one of the following:
    ...
    *   :class:`AbstractModel` for abstract super classes meant to be shared by
    ...
    the :attr:`~autanac.models.BaseModel._register` attribute may be set to False.
    """

    _auto = False
    """Whether a database table should be created (default: ``True``).
    If set to ``False``, override :meth:`~autanac.models.BaseModel.init`
    to create the database table.

    .. tip:: To create a model without any table, inherit
            from :class:`~autanac.models.AbstractModel`.
    """
    ...
    .. warning::
      ...
      :attr:`~autanac.models.Model._inherits`-ed models, the inherited field will
    ...
    def compute_concurrency_field(self):
        for record in self:
            record[self.CONCURRENCY_CHECK_FIELD] = autanac.fields.Datetime.now()

    @api.depends('create_date', 'write_date')
    def compute_concurrency_field_with_access(self):
        for record in self:
            record[self.CONCURRENCY_CHECK_FIELD] = \
                record.write_date or record.create_date or autanac.fields.Datetime.now()
    ...
    def _add_fake_fields(self, fields):
        from autanac.fields import Char, Integer
    ...
    def _get_default_kanban_view(self):
        ...
        content_div = E.div(field, {'class': "a_kanban_card_content"})
...
%(fields_list)s""") % {
    ...
    def _check_company(self, fnames=None):
        ...
        :raises UserError: if the `company_id` of the value of any field is not
            ...
            :class:`~autanac.addons.base.models.res_company`).

        For :class:`~autanac.addons.base.models.res_users` relational fields,
    ...
    def write(self, vals):
        ...
        * For numeric fields (:class:`~autanac.fields.Integer`,
          :class:`~autanac.fields.Float`) the value should be of the
          corresponding type
        * For :class:`~autanac.fields.Boolean`, the value should be a
          :class:`python:bool`
        * For :class:`~autanac.fields.Selection`, the value should match the
          selection values (generally :class:`python:str`, sometimes
          :class:`python:int`)
        * For :class:`~autanac.fields.Many2one`, the value should be the
        ...
        * Other non-relational fields use a string for value
          ...
          .. danger::
              ...
              :class:`~autanac.fields.Date` and
              :class:`~autanac.fields.Datetime` fields use strings as values
              (written and read) rather than :class:`~python:datetime.date` or
              :class:`~python:datetime.datetime`. These date strings are
              UTC-only and formatted according to
              :const:`autanac.tools.misc.DEFAULT_SERVER_DATE_FORMAT` and
              :const:`autanac.tools.misc.DEFAULT_SERVER_DATETIME_FORMAT`
        * .. _openerp/models/relationals/format:

          :class:`~autanac.fields.One2many` and
          :class:`~autanac.fields.Many2many` use a special "commands" format to
    ...
    def ensure_one(self):
        ...
        :raise autanac.exceptions.ValueError: ``len(self) != 1``
        """
        try:
    ...
    def with_env(self, env):
        ...
        :param env:
        :type env: :class:`~autanac.api.Environment`
        ...
        for field, value in sorted(field_values, key=is_monetary):
            ...
            if field.relational:
                ...
                for invf in self._field_inverses[field]:
                    ...
                    # /home/dle/src/autanac/master-nochange-fp/autanac/addons/test_new_api/tests/test_new_fields.py
...
class Model(AbstractModel):
    """ Main super-class for regular database-persisted autanac models.

    autanac models are created by inheriting from this class::
