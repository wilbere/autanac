...
# *broken*
NO_POSTMORTEM = (autanac.exceptions.except_orm,
                 autanac.exceptions.AccessDenied,
                 autanac.exceptions.Warning,
                 autanac.exceptions.RedirectWarning)
    ...
    try:
        ...
        if rpc_request_flag or rpc_response_flag:
            ...
            if rpc_request and rpc_response_flag:
                autanac.netsvc.log(rpc_request, logging.DEBUG, '%s.%s' % (service_name, method), replace_request_password(params))

        threading.current_thread().uid = None
        threading.current_thread().dbname = None
        if service_name == 'common':
            dispatch = autanac.service.common.dispatch
        elif service_name == 'db':
            dispatch = autanac.service.db.dispatch
        elif service_name == 'object':
            dispatch = autanac.service.model.dispatch
        ...
        if rpc_request_flag or rpc_response_flag:
            ...
            if rpc_response_flag:
                autanac.netsvc.log(rpc_response, logging.DEBUG, logline, result)
            else:
                autanac.netsvc.log(rpc_request, logging.DEBUG, logline, replace_request_password(params), depth=1)

        return result
    except NO_POSTMORTEM:
        raise
    except autanac.exceptions.DeferredException as e:
        _logger.exception(autanac.tools.exception_to_unicode(e))
        autanac.tools.debugger.post_mortem(autanac.tools.config, e.traceback)
        raise
    except Exception as e:
        _logger.exception(autanac.tools.exception_to_unicode(e))
        autanac.tools.debugger.post_mortem(autanac.tools.config, sys.exc_info())
        raise
...
class WebRequest(object):
    """ Parent class for all autanac Web request types, mostly deals with
    ...
    def __init__(self, httprequest):
        ...
        # dispatching phase in autanac.service.wsgi_server.application
        if self.db:
    ...
    @property
    def cr(self):
        """ :class:`~autanac.sql_db.Cursor` initialized for the current method call.
    ...
    @property
    def env(self):
        """ The :class:`~autanac.api.Environment` bound to current request. """
        if self._env is None:
            self._env = autanac.api.Environment(self.cr, self.uid, self.context)
        return self._env
    ...
    def _handle_exception(self, exception):
        ...
        if not isinstance(exception, NO_POSTMORTEM) \
            ...
            autanac.tools.debugger.post_mortem(
                autanac.tools.config, sys.exc_info())
    ...
    def registry(self):
        ...
        .. deprecated:: 8.0
            ...
            use :attr:`.env`
        """
        return autanac.registry(self.db)
...
def route(route=None, **kw):
    ...
    .. admonition:: CSRF Protection
        ...
        autanac implements token-based `CSRF protection
        ...
        * if the form is generated in Python, a csrf token is
          ...
          <autanac.http.WebRequest.csrf_token`, the
          :data:`~autanac.http.request` object is available by default
        ...
        * if the endpoint can be called by external parties (not from
          autanac) as e.g. it is a REST API or a `webhook
    ...
    def _handle_exception(self, exception):
        ...
        try:
            ...
            if not isinstance(exception, SessionExpiredException):
                ...
                elif isinstance(exception, (autanac.exceptions.Warning, autanac.exceptions.except_orm,
            ...
            if isinstance(exception, AuthenticationError):
                error['code'] = 100
                error['message'] = "autanac Session Invalid"
            if isinstance(exception, SessionExpiredException):
                error['code'] = 100
                error['message'] = "autanac Session Expired"
...
def serialize_exception(e):
    ...
    elif isinstance(e, autanac.exceptions.Warning):
    ...
    elif isinstance(e, autanac.exceptions.ValidationError):
        tmp["exception_type"] = "validation_error"
    elif isinstance(e, autanac.exceptions.except_orm):
...
autanac URLs are CSRF-protected by default (when accessed with unsafe
HTTP methods). See
https://www.autanac.com/documentation/13.0/reference/http.html#csrf for
more details.

* if this endpoint is accessed through autanac via py-QWeb form, embed a CSRF
...
class ControllerType(type):
    def __init__(cls, name, bases, attrs):
        ...
        if not class_path[:2] == ["autanac", "addons"]:
            module = ""
        else:
...
def _generate_routing_rules(modules, nodb_only, converters=None):
    def get_subclasses(klass):
        def valid(c):
            return c.__module__.startswith('autanac.addons.') and c.__module__.split(".")[2] in modules
...
class OpenERPSession(werkzeug.contrib.sessions.Session):
    ...
    def authenticate(self, db, login=None, password=None, uid=None):
        ...
        if uid is None:
            ...
            uid = autanac.registry(db)['res.users'].authenticate(db, login, password, env)
        else:
    ...
    def check_security(self):
        ...
        env = autanac.api.Environment(request.cr, self.uid, self.context)
...
class Response(werkzeug.wrappers.Response):
    ...
    def render(self):
        ...
        """
        env = request.env(user=self.uid or request.uid or autanac.SUPERUSER_ID)
...
class Root(object):
    ...
    def session_store(self):
        # Setup http sessions
        path = autanac.tools.config.session_dir
    ...
    def nodb_routing_map(self):
        ...
        for url, endpoint, routing in autanac.http._generate_routing_rules([''] + autanac.conf.server_wide_modules, True):
    ...
    def load_addons(self):
        ...
        statics = {}
        for addons_path in autanac.addons.__path__:
    ...
    def dispatch(self, environ, start_response):
        ...
        try:
            ...
            with request:
                ...
                if db:
                    try:
                        autanac.registry(db).check_signaling()
                        with autanac.tools.mute_logger('autanac.sql_db'):
...
def db_list(force=False, httprequest=None):
    dbs = autanac.service.db.list_dbs(force)
...
def db_filter(dbs, httprequest=None):
    ...
    if d == "www" and r:
        d = r.partition('.')[0]
    if autanac.tools.config['dbfilter']:
        d, h = re.escape(d), re.escape(h)
        r = autanac.tools.config['dbfilter'].replace('%h', h).replace('%d', d)
        dbs = [i for i in dbs if re.match(r, i)]
    elif autanac.tools.config['db_name']:
        # In case --db-filter is not provided and --database is passed, autanac will
        # use the value of --database as a comma seperated list of exposed databases.
        exposed_dbs = set(db.strip() for db in autanac.tools.config['db_name'].split(','))
...
def send_file(filepath_or_fp, mimetype=None, as_attachment=False, filename=None, mtime=None,
    ...
    if isinstance(mtime, str):
        try:
            server_format = autanac.tools.misc.DEFAULT_SERVER_DATETIME_FORMAT
...
def content_disposition(filename):
    filename = autanac.tools.ustr(filename)
