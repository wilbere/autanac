...
from autanac import fields, tools, SUPERUSER_ID
from autanac.tools.pycompat import to_text
from autanac.http import request
from autanac.modules.module import get_resource_path
from .qweb import escape
import psycopg2
from autanac.tools import func, misc
        ...
        return """
            (function (message) {
                ...
                async function onDOMContentLoaded() {
                    var autanac = window.top.autanac;
                    if (!autanac || !autanac.define) {
                        useAlert();
                        return;
                    }
                    ...
                    await new Promise(resolve => {
                        const noLazyTimeout = setTimeout(() => resolve(), 10); // 10 since need to wait for promise resolutions of autanac.define
                        autanac.define('AssetsBundle.PotentialLazyLoading', function (require) {
                            'use strict';
                    ...
                    var alertTimeout = setTimeout(useAlert, 10); // 10 since need to wait for promise resolutions of autanac.define
                    autanac.define('AssetsBundle.ErrorMessage', function (require) {
                        'use strict';
    ...
    def compile_css(self, compiler, source):
        ...
        def sanitize(matchobj):
            ...
            if '.' not in ref and line not in imports and not ref.startswith(('.', '/', '~')):
                imports.append(line)
                return line
            msg = "Local import '%s' is forbidden for security reasons. Please remove all @import {your_file} imports in your custom files. In autanac you have to import all files in the assets, and not through the @import statement." % ref
            _logger.warning(msg)
