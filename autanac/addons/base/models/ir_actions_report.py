...
from autanac.exceptions import UserError, AccessError
...
from autanac.osv.expression import NEGATIVE_TERM_OPERATORS, FALSE_DOMAIN
...
# here to init the T1 fonts cache at the start-up of autanac so that rendering of barcode in multiple
...
# Check the presence of Wkhtmltopdf and return its version at autanac start-up
...
try:
    ...
    if match:
        ...
        if config['workers'] == 1:
            _logger.info('You need to start autanac with at least two workers to print a pdf version of the reports.')
