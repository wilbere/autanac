...
from autanac.exceptions import MissingError, UserError, ValidationError, AccessError
from autanac.osv import expression
from autanac.tools.safe_eval import safe_eval, test_python_expr
from autanac.tools import wrap_module
...
class IrActionsServer(models.Model):
    ...
    Since autanac 8.0 a button 'Create Menu Action' button is available on the
...
#  - env: autanac Environment on which the action is triggered
#  - model: autanac Model of the record on which the action is triggered; is a void recordset
...
# To return an action, assign: action = {...}\n\n\n\n"""
    ...
    def _get_eval_context(self, action=None):
        ...
        eval_context.update({
            ...
            'model': model,
            # Exceptions
            'Warning': autanac.exceptions.Warning,
            # record
            'record': record,
            'records': records,
            # helpers
