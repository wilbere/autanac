...
import autanac
from autanac import api, fields, models, modules, tools, _
from autanac.addons.base.models.ir_model import MODULE_UNINSTALL_FLAG
from autanac.exceptions import AccessDenied, UserError
from autanac.osv import expression
from autanac.tools.parse_version import parse_version
from autanac.tools.misc import topological_sort
...
class Module(models.Model):
    ...
    license = fields.Selection([
        ...
        ('OEEL-1', 'autanac Enterprise Edition License v1.0'),
        ('OPL-1', 'autanac Proprietary License v1.0'),
    ...
    to_buy = fields.Boolean('autanac Enterprise Module', default=False)
    ...
    def install_from_urls(self, urls):
        ...
        OPENERP = autanac.release.product_name.lower()
        ...
        try:
            ...
            if urls.get(OPENERP):
                ...
                # copy all modules in the SERVER/autanac/addons directory to the new "autanac" module (except base itself)
                for d in os.listdir(base_path):
                    if d != 'base' and os.path.isdir(os.path.join(base_path, d)):
                        destdir = os.path.join(tmp, OPENERP, 'addons', d)    # XXX 'autanac' subdirectory ?
                ...
                _logger.info('Copy downloaded module `autanac` to `%s`', server_dir)
            ...
            if installed or to_install:
                ...
                self._cr.commit()
                autanac.service.server.restart()
                return {
                    'type': 'ir.actions.client',
                    'tag': 'home',
                    'params': {'wait': True},
    ...
    def get_apps_server(self):
        return tools.config.get('apps_server', 'https://apps.autanac.com/apps')
