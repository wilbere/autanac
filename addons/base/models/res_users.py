...
from autanac import api, fields, models, tools, SUPERUSER_ID, _
from autanac.exceptions import AccessDenied, AccessError, UserError, ValidationError
from autanac.http import request
from autanac.osv import expression
from autanac.service.db import check_super
from autanac.tools import partition, collections, lazy_property
...
class Users(models.Model):
    ...
    def unlink(self):
        if SUPERUSER_ID in self.ids:
            raise UserError(_('You can not remove the admin user as it is used internally for resources created by autanac (updates, module installation, ...)'))
        db = self._cr.dbname
        for id in self.ids:
            self.__uid_cache[db].pop(id, None)
        self._invalidate_session_cache()
    ...
    def change_password(self, old_passwd, new_passwd):
        ...
        :return: True
        :raise: autanac.exceptions.AccessDenied when old password is wrong
        :raise: autanac.exceptions.UserError when new password is not set or empty
        """
        ...
        if self._on_login_cooldown(failures, previous):
            ...
            if ipaddress.ip_address(source).is_private:
                _logger.warn(
                    ...
                    "https://www.autanac.com/documentation/0.2/setup/deploy.html#https for details.",
                    source
                )
