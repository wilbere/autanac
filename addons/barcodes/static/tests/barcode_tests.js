autanac.define('barcodes.tests', function (require) {
"use strict";
...
QUnit.test('edit, save and cancel buttons', async function (assert) {
    ...
    await testUtils.fields.editInput(form.$('.a_field_widget'), 'test');
    // O-CMD.SAVE
...
QUnit.test('pager buttons', async function (assert) {
    ...
    assert.strictEqual(form.$('.a_field_widget').text(), 'Large Cabinet');
    // O-CMD.PAGER-NEXT
    _.each(["O","-","C","M","D",".","N","E","X","T","Enter"], triggerKeypressEvent);
    await testUtils.nextTick();
    assert.strictEqual(form.$('.a_field_widget').text(), 'Cabinet with Doors');
    // O-CMD.PAGER-PREV
...
QUnit.test('widget field_float_scannable', async function (assert) {
    ...
    assert.strictEqual(form.$('.a_field_widget[name=int_field]').text(), '4',
        "should display the correct value in readonly");

    await testUtils.form.clickEdit(form);

    assert.strictEqual(form.$('.a_field_widget[name=int_field]').val(), '4',
    ...
    form.$('.a_field_widget[name=int_field]').focus();
    assert.strictEqual(form.$('.a_field_widget[name=int_field]').get(0), document.activeElement,
        "int field should be focused");
    form.$('.a_field_widget[name=int_field]').trigger({type: 'keypress', which: 50, keyCode: 50}); // 2
    await testUtils.nextTick();
    assert.strictEqual(form.$('.a_field_widget[name=int_field]').get(0), document.activeElement,
        "int field should still be focused");
    form.$('.a_field_widget[name=int_field]').trigger({type: 'keypress', which: 54, keyCode: 54}); // 6
    await testUtils.nextTick();
    assert.strictEqual(form.$('.a_field_widget[name=int_field]').get(0), document.activeElement,
        "int field should still be focused");

    setTimeout(async function () {
        assert.strictEqual(form.$('.a_field_widget[name=int_field]').val(), '426',
            "should display the correct value in edit");
        assert.strictEqual(form.$('.a_field_widget[name=int_field]').get(0), document.activeElement,
        "int field should still be focused");
        ...
        form.$('.a_field_widget[name=int_field]').trigger('change'); // should trigger the onchange
        await testUtils.nextTick();
...
QUnit.test('specification of widget barcode_handler', async function (assert) {
    ...
    await testUtils.nextTick();
    assert.strictEqual(form.$('.a_data_row:first .a_data_cell:nth(1)').text(), '1',
        "quantity of line one should have been incremented");
    _.each(['1','2','3','4','5','6','7','8','9','0','Enter'], triggerKeypressEvent);
    await testUtils.nextTick();
    assert.strictEqual(form.$('.a_data_row:first .a_data_cell:nth(1)').text(), '2',
        "quantity of line one should have been incremented");

    // scan once product 2
    _.each(['0','9','8','7','6','5','4','3','2','1','Enter'], triggerKeypressEvent);
    await testUtils.nextTick();
    assert.strictEqual(form.$('.a_data_row:nth(1) .a_data_cell:nth(1)').text(), '1',
        "quantity of line one should have been incremented");
...
QUnit.test('specification of widget barcode_handler with keypress and notifyChange', async function (assert) {
    ...
    setTimeout(async function () {
        ...
        await testUtils.nextTick();
        assert.strictEqual(form.$('.a_data_row .a_data_cell:nth(1)').text(), '7',
