...
from autanac.tests import tagged
...
class TestProductAttributeValueConfig(TestProductAttributeValueSetup):
    ...
    def test_constraints(self):
        ...
        with mute_logger('autanac.sql_db'), self.assertRaises(IntegrityError, msg="can't have two values with the same name for the same attribute"):
            self.env['product.attribute.value'].create({
                'name': '32 GB',
                'attribute_id': self.ram_attribute.id,
            })
