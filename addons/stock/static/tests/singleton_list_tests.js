autanac.define('web.singleton_list_tests', function (require) {
"use strict";
...
QUnit.module('Views', {
    ...
    QUnit.test('Create new record correctly', async function (assert) {
        ...
        assert.containsN(list, '.a_data_row', 4, "should now have 4 records");
        list.destroy();
    });
    ...
    QUnit.test('Don\'t duplicate record', async function (assert) {
        ...
        assert.containsN(list, '.a_data_row', 3, "should still have 3 records");
    ...
    QUnit.test('Don\'t raise error when trying to create duplicate line', async function (assert) {
       ...
       */
        ...
        assert.containsN(list, '.a_data_row', 1, "should have 1 records");
        ...
        assert.containsN(list, '.a_data_row', 1, "should now have 1 records");
        // ... and verify modification was occured.
        var nameField = list.$('td[title="' + name + '"]');
        var ageField = nameField.parent().find('.o_list_number');
        assert.strictEqual(ageField.text(), age, "The age field must be updated");
    ...
    QUnit.test('Refresh the list only when needed', async function (assert) {
        ...
        // Modify Record
        await testUtils.dom.click(list.$('.a_data_row:nth-child(2) > .o_list_number'));
        var $input = $('.a_selected_row input[name=age]');
        ...
        // Add existing record
        await testUtils.dom.click($('.o_list_button_add'));
        $input = $('.a_selected_row input[name=name]');
    ...
    QUnit.test('Work in grouped list', async function (assert) {
        ...
        await testUtils.fields.triggerKeydown($input, 'tab');
        $input = $('.a_selected_row input[name=job]');
        ...
        var ageCell = $('tr.a_data_row td.o_list_number').first();
        assert.strictEqual(ageCell.text(), "55",
            "age of the record must be updated");
        // Edits the freshly created record...
        await testUtils.dom.click(list.$('tr.a_data_row td.o_list_number').eq(1));
        $input = $('.a_selected_row input[name=age]');
        ...
        ageCell = $('tr.a_data_row td.o_list_number').first();
