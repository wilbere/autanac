...
from autanac import _, SUPERUSER_ID
from autanac.http import request
from autanac.addons.web.controllers import main as web

def _admin_password_warn(uid):
    """ Admin still has `admin` password, flash a message via chatter.

    Uses a private mail.channel from the system (/ autanacbot) to the user, as
