autanac.define('mail.chatter_tests', function (require) {
"use strict";
...
QUnit.test('chatter in create mode', async function (assert) {
    ...
    assert.hasClass(form.$el.find('.a_form_view'),'o_form_editable',
...
QUnit.test('kanban activity widget with no activity', async function (assert) {
    ...
    var $record = kanban.$('.a_kanban_record').first();
...
QUnit.test('kanban activity widget with an activity', async function (assert) {
    ...
    $record = kanban.$('.a_kanban_record').first(); // the record widget has been reset
...
QUnit.test('list activity exception widget with activity', async function (assert) {
    ...
    assert.containsN(list, '.a_data_row', 2, "should have two records");
    assert.doesNotHaveClass(list.$('.a_data_row:eq(0) .o_activity_exception_cell div'), 'fa-warning',
        "there is no any exception activity on record");
    assert.hasClass(list.$('.a_data_row:eq(1) .o_activity_exception_cell div'), 'fa-warning',
...
QUnit.test('chatter: post, receive and star messages', async function (assert) {
    ...
    var form = await createView({
        ...
        mockRPC: function (route, args) {
            ...
            if (args.method === 'get_mention_suggestions') {
                ...
                return Promise.resolve([[{email: "test@autanac.com", id: 1, name: "Test User"}], []]);
            }
    ...
    assert.strictEqual(form.$('.o_mention_proposition .o_mention_info').text(), '(test@autanac.com)',
        "suggestion should be displayed correctly");
    //cleanup
    form.destroy();
});
...
QUnit.test('chatter: discard changes on message post with post_refresh "recipients"', async function (assert) {
    ...
    var form = await createView({
        ...
        mockRPC: function (route, args) {
            ...
            if (args.method === 'get_mention_suggestions') {
                getSuggestionsDef.resolve();
                return Promise.resolve([[{email: "me@autanac.com", id: 42, name: "Me"}], []]);
            }
            if (args.method === 'message_format') {
                var requested_msgs = _.filter(messages, function (msg) {
                    return _.contains(args.args[0], msg.id);
...
QUnit.test('form activity widget: schedule activity does not discard changes', async function (assert) {
    ...
    await testUtils.fields.editInput(form.$('.a_field_widget[name=foo]'), 'new value');
...
QUnit.test('many2many_tags_email widget can load more than 40 records', async function (assert) {
    ...
    assert.strictEqual(form.$('.a_field_widget[name="partner_ids"] .badge').length, 100);
    ...
    assert.strictEqual(form.$('.a_field_widget[name="partner_ids"] .badge').length, 101);
