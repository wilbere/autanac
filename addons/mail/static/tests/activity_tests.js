autanac.define('mail.activity_view_tests', function (require) {
'use strict';
...
QUnit.test('activity view: simple activity rendering', async function (assert) {
    ...
    assert.containsOnce($th1, '.a_kanban_counter', 'should contain a progressbar in header of first column');
    assert.hasAttrValue($th1.find('.a_kanban_counter_progress .progress-bar:first'), 'data-original-title', '1 Planned',
        'the counter progressbars should be correctly displayed');
    assert.hasAttrValue($th1.find('.a_kanban_counter_progress .progress-bar:nth-child(2)'), 'data-original-title', '1 Today',
        'the counter progressbars should be correctly displayed');
    ...
    assert.hasAttrValue($th2.find('.a_kanban_counter_progress .progress-bar:nth-child(3)'), 'data-original-title', '1 Overdue',
        'the counter progressbars should be correctly displayed');
    assert.containsNone(activity, 'table thead tr:first th:nth-child(4) .a_kanban_counter',
...
QUnit.test('activity view: search more to schedule an activity for a record of a respecting model', async function (assert) {
    ...
    var $modal = $('.modal-lg');
    assert.strictEqual($modal.find('.a_data_row').length, 3, "all tasks should be available to select");
    // select a record to schedule an activity for it (this triggers a do_action)
    testUtils.dom.click($modal.find('.a_data_row:last'));
...
QUnit.test('Activity view: discard an activity creation dialog', async function (assert) {
    ...
    await testUtils.dom.click(actionManager.$('.o_activity_view .a_data_row .o_activity_empty_cell')[0]);
    assert.containsOnce(
        $,
