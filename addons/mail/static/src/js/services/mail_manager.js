autanac.define('mail.Manager', function (require) {
"use strict";
...
var MailManager =  AbstractService.extend({
    ...
    _isReady: false,
    _autanacBOT_ID: ["autanacBOT", "autanacBOT"], // authorID for transient messages
    ...
    /**
     * Get the autanacBot ID, which is the default authorID for transient messages
     *
     * @returns {Array<string>}
     */
    getautanacbotID: function () {
        return this._autanacBOT_ID;
    },
    /**
                ...
                if (
                    ...
                    if (thread.isTwoUserThread() && options.showNotification) {
                        ...
                        prom.then(function () {
                            ...
                            if (!self.call('bus_service', 'isautanacFocused') || !query.isVisible) {
    ...
    _initializeInternalState: function () {
        ...
        // # of message received when autanac is out of focus
    ...
    _notifyIncomingMessage: function (message) {
        if (this.call('bus_service', 'isautanacFocused')) {
            // no need to notify
            return;
        }
