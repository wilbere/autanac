autanac.define('mail.ActivityRenderer', function (require) {
"use strict";
...
var ActivityRenderer = AbstractRenderer.extend({
    ...
    _renderRow: function (resId) {
        ...
        return Promise.all(defs).then(function () {
            var $tr = $('<tr/>', {class: 'a_data_row'}).attr('data-res-id', resId)
                .append($nameTD)
                .append($cells);
            return $tr;
        });
     ...
     */
     _onSetProgressBarState: function (ev) {
        ...
        this.$('.a_kanban_counter_progress div').removeClass('active progress-bar-striped');
