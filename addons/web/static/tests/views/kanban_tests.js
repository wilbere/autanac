autanac.define('web.kanban_tests', function (require) {
"use strict";
...
QUnit.module('Views', {
    ...
    QUnit.test('basic ungrouped rendering', async function (assert) {
        ...
        var kanban = await createView({
            ...
            model: 'partner',
            data: this.data,
            arch: '<kanban class="a_kanban_test"><templates><t t-name="kanban-box">' +
                    '<div>' +
        ...
        assert.hasClass(kanban.$('.a_kanban_view'), 'a_kanban_ungrouped');
        assert.hasClass(kanban.$('.a_kanban_view'), 'a_kanban_test');
        assert.containsN(kanban, '.a_kanban_record:not(.a_kanban_ghost)', 4);
        assert.containsN(kanban,'.a_kanban_ghost', 6);
        assert.containsOnce(kanban, '.a_kanban_record:contains(gnap)');
        kanban.destroy();
    });
    ...
    QUnit.test('basic grouped rendering', async function (assert) {
        ...
        var kanban = await createView({
            ...
            model: 'partner',
            data: this.data,
            arch: '<kanban class="a_kanban_test">' +
                        '<field name="bar"/>' +
        ...
        assert.hasClass(kanban.$('.a_kanban_view'), 'a_kanban_grouped');
        assert.hasClass(kanban.$('.a_kanban_view'), 'a_kanban_test');
        assert.containsN(kanban, '.a_kanban_group', 2);
        assert.containsOnce(kanban, '.a_kanban_group:nth-child(1) .a_kanban_record');
        assert.containsN(kanban, '.a_kanban_group:nth-child(2) .a_kanban_record', 3);

        // check available actions in kanban header's config dropdown
        assert.containsOnce(kanban, '.a_kanban_header:first .a_kanban_config .a_kanban_toggle_fold');
        assert.containsNone(kanban, '.a_kanban_header:first .a_kanban_config .o_column_edit');
        assert.containsNone(kanban, '.a_kanban_header:first .a_kanban_config .o_column_delete');
        assert.containsNone(kanban, '.a_kanban_header:first .a_kanban_config .o_column_archive_records');
        assert.containsNone(kanban, '.a_kanban_header:first .a_kanban_config .o_column_unarchive_records');
    ...
    QUnit.test('basic grouped rendering with active field (archivable by default)', async function (assert) {
        ...
        assert.containsOnce(kanban, '.a_kanban_header:first .a_kanban_config .o_column_archive_records');
        assert.containsOnce(kanban, '.a_kanban_header:first .a_kanban_config .o_column_unarchive_records');
        assert.deepEqual(kanban.exportState().resIds, envIDs);

        // archive the records of the first column
        assert.containsN(kanban, '.a_kanban_group:last .a_kanban_record', 3);

        testUtils.kanban.toggleGroupSettings(kanban.$('.a_kanban_group:last'));
        await testUtils.dom.click(kanban.$('.a_kanban_group:last .o_column_archive_records'));
        assert.containsOnce(document.body, '.modal', "a confirm modal should be displayed");
        await testUtils.modal.clickButton('Cancel');
        assert.containsN(kanban, '.a_kanban_group:last .a_kanban_record', 3, "still last column should contain 3 records");
        testUtils.kanban.toggleGroupSettings(kanban.$('.a_kanban_group:last'));
        await testUtils.dom.click(kanban.$('.a_kanban_group:last .o_column_archive_records'));
        assert.ok($('.modal').length, 'a confirm modal should be displayed');
        await testUtils.modal.clickButton('Ok');
        assert.containsN(kanban, '.a_kanban_view .a_kanban_group', 1, "there should be one remaining column");
        envIDs = [4];
        assert.deepEqual(kanban.exportState().resIds, envIDs);
        kanban.destroy();
    });
    ...
    QUnit.test('basic grouped rendering with active field and archive enabled (archivable true)', async function (assert) {
        ...
        var kanban = await createView({
            ...
            model: 'partner',
            data: this.data,
            arch: '<kanban class="a_kanban_test" archivable="true">' +
                        '<field name="active"/>' +
                        '<field name="bar"/>' +
                        '<templates><t t-name="kanban-box">' +
                        '<div><field name="foo"/></div>' +
        ...
        // check archive/restore all actions in kanban header's config dropdown
        assert.ok(kanban.$('.a_kanban_header:first .a_kanban_config .o_column_archive_records').length, "should be able to archive all the records");
        assert.ok(kanban.$('.a_kanban_header:first .a_kanban_config .o_column_unarchive_records').length, "should be able to restore all the records");
        ...
        assert.containsNone(kanban, '.a_kanban_group:last .a_kanban_record', "last column should not contain any records");
        kanban.destroy();
    });
    ...
    QUnit.test('basic grouped rendering with active field and hidden archive buttons (archivable false)', async function (assert) {
        ...
        var kanban = await createView({
            ...
            model: 'partner',
            data: this.data,
            arch: '<kanban class="a_kanban_test" archivable="false">' +
                        '<field name="active"/>' +
                        '<field name="bar"/>' +
                        '<templates><t t-name="kanban-box">' +
                        '<div><field name="foo"/></div>' +
        ...
        // check archive/restore all actions in kanban header's config dropdown
        assert.strictEqual(
            kanban.$('.a_kanban_header:first .a_kanban_config .o_column_archive_records').length, 0,
            "should not be able to archive all the records");
        assert.strictEqual(
            kanban.$('.a_kanban_header:first .a_kanban_config .o_column_unarchive_records').length, 0,
    ...
    QUnit.test('context can be used in kanban template', async function (assert) {
        ...
        assert.strictEqual(form.$('.a_kanban_record:not(.a_kanban_ghost)').length, 1,
            "there should be one record");
        assert.strictEqual(form.$('.a_kanban_record span:contains(yop)').length, 1,
    ...
    QUnit.test('pager, ungrouped, with limit set on arch and given in options', async function (assert) {
        ...
        var kanban = await createView({
            ...
            model: 'partner',
            data: this.data,
            arch: '<kanban class="a_kanban_test" limit="3">' +
                        '<templates><t t-name="kanban-box">' +
                        '<div><field name="foo"/></div>' +
                    '</t></templates></kanban>',
            mockRPC: function (route, args) {
    ...
    QUnit.test('create in grouped on m2o', async function (assert) {
        ...
        var kanban = await createView({
            ...
            model: 'partner',
            data: this.data,
            arch: '<kanban class="a_kanban_test" on_create="quick_create">' +
        ...
        assert.hasClass(kanban.$('.a_kanban_view'),'ui-sortable',
        ...
        assert.hasClass(kanban.$('.a_kanban_view > div:last'),'o_column_quick_create',
        ...
        assert.hasClass(kanban.$('.a_kanban_group:first() > div:nth(1)'),'a_kanban_quick_create',
    ...
    QUnit.test('create in grouped on char', async function (assert) {
        ...
        assert.doesNotHaveClass(kanban.$('.a_kanban_view'), 'ui-sortable',
            "columns aren't sortable when not grouped by a m2o field");
        assert.containsN(kanban, '.a_kanban_group', 3, "should have " + 3 + " columns");
        assert.strictEqual(kanban.$('.a_kanban_group:first() .o_column_title').text(), "yop",
            "'yop' column should be the first column");
        assert.doesNotHaveClass(kanban.$('.a_kanban_view > div:last'), 'o_column_quick_create',
    ...
    QUnit.test('quick create record without quick_create_view', async function (assert) {
        ...
        assert.containsOnce(kanban, '.a_kanban_group:first .a_kanban_record',
        ...
        var $quickCreate = kanban.$('.a_kanban_group:first .a_kanban_quick_create');

        assert.strictEqual($quickCreate.length, 1,
            "should have a quick create element in the first column");
        assert.strictEqual($quickCreate.find('.a_form_view.o_xxs_form_view').length, 1,
        ...
        await testUtils.dom.click($quickCreate.find('button.a_kanban_add'));

        assert.containsN(kanban, '.a_kanban_group:first .a_kanban_record', 2,
    ...
    QUnit.test('quick create record with quick_create_view', async function (assert) {
        ...
        assert.strictEqual($quickCreate.find('.a_field_widget').length, 3,
            "should have rendered three widgets");

        // fill the quick create and validate
        await testUtils.fields.editInput($quickCreate.find('.a_field_widget[name=foo]'), 'new partner');
        await testUtils.fields.editInput($quickCreate.find('.a_field_widget[name=int_field]'), '4');
        await testUtils.dom.click($quickCreate.find('.a_field_widget[name=state] .o_priority_star:first'));
        await testUtils.dom.click($quickCreate.find('button.a_kanban_add'));
    ...
    QUnit.test('quick create record in grouped on m2o (no quick_create_view)', async function (assert) {
        ...
        assert.containsN(kanban, '.a_kanban_group:first .a_kanban_record', 3,
    ...
    QUnit.test('quick create record with default values and onchanges', async function (assert) {
        ...
        assert.strictEqual($quickCreate.length, 1,
            "should have a quick create element in the first column");
        assert.strictEqual($quickCreate.find('.a_field_widget[name=int_field]').val(), '4',
            "default value should be set");

        // fill the 'foo' field -> should trigger the onchange
        await testUtils.fields.editInput($quickCreate.find('.a_field_widget[name=foo]'), 'new partner');

        assert.strictEqual($quickCreate.find('.a_field_widget[name=int_field]').val(), '8',
    ...
    QUnit.test('quick create record with quick_create_view: modifiers', async function (assert) {
        ...
        // create a new record
        await testUtils.dom.click(kanban.$('.a_kanban_group:first .a_kanban_quick_add'));
        var $quickCreate = kanban.$('.a_kanban_group:first .a_kanban_quick_create');

        assert.hasClass($quickCreate.find('.a_field_widget[name=foo]'),'o_required_modifier',
            "foo field should be required");
        assert.hasClass($quickCreate.find('.a_field_widget[name=int_field]'),'o_invisible_modifier',
            "int_field should be invisible");

        // fill 'foo' field
        await testUtils.fields.editInput($quickCreate.find('.a_field_widget[name=foo]'), 'new partner');

        assert.doesNotHaveClass($quickCreate.find('.a_field_widget[name=int_field]'), 'o_invisible_modifier',
    ...
    QUnit.test('quick create record and change state in grouped mode', async function (assert) {
        ...
        await testUtils.dom.click(kanban.$('.a_kanban_header .a_kanban_quick_add i').first());
        var $quickAdd = kanban.$('.a_kanban_quick_create');
        $quickAdd.find('.o_input').val('Test');
        await testUtils.dom.click($quickAdd.find('.a_kanban_add'));
    ...
    QUnit.test('quick create record: cancel and validate without using the buttons', async function (assert) {
        ...
        var $quickCreate = kanban.$('.a_kanban_quick_create');
        ...
        assert.containsNone(kanban, '.a_kanban_quick_create',
    ...
    QUnit.test('quick create record: validate with ENTER', async function (assert) {
        ...
        assert.containsN(kanban, '.a_kanban_record', 4,
        ...
        assert.containsN(kanban, '.a_kanban_record', 5,
            "should have created a new record");
        assert.strictEqual(kanban.$('.a_kanban_quick_create input[name=foo]').val(), '',
    ...
    QUnit.test('quick create record: prevent multiple adds with ENTER', async function (assert) {
        ...
        await testUtils.fields.editAndTrigger(
            kanban.$('.a_kanban_quick_create').find('input[name=foo]'),
            'new partner',
        ...
        assert.strictEqual(kanban.$('.a_kanban_quick_create input[name=foo]').val(), 'new partner',
            "quick create should not be empty yet");
        assert.hasClass(kanban.$('.a_kanban_quick_create'), 'o_disabled',
        ...
        assert.strictEqual(kanban.$('.a_kanban_quick_create input[name=foo]').val(), '',
            "quick create should now be empty");
        assert.doesNotHaveClass(kanban.$('.a_kanban_quick_create'), 'o_disabled',
    ...
    QUnit.test('quick create record: prevent multiple adds with Add clicked', async function (assert) {
        ...
        await testUtils.dom.click(kanban.$('.a_kanban_header .a_kanban_quick_add i').first());
        await testUtils.fields.editInput(kanban.$('.a_kanban_quick_create').find('input[name=foo]'), 'new partner');
        await testUtils.dom.click(kanban.$('.a_kanban_quick_create').find('.a_kanban_add'));
        await testUtils.dom.click(kanban.$('.a_kanban_quick_create').find('.a_kanban_add'));
    ...
    QUnit.test('quick create when first column is folded', async function (assert) {
        ...
        assert.doesNotHaveClass(kanban.$('.a_kanban_group:first'), 'o_column_folded',
            "first column should not be folded");

        // fold the first column
        testUtils.kanban.toggleGroupSettings(kanban.$('.a_kanban_group:first'));
        await testUtils.dom.click(kanban.$('.a_kanban_group:first .a_kanban_toggle_fold'));

        assert.hasClass(kanban.$('.a_kanban_group:first'),'o_column_folded',
    ...
    QUnit.test('quick create record: cancel when not dirty', async function (assert) {
        ...
        kanban.$('.a_kanban_quick_create input').trigger($.Event('keydown', {
            keyCode: $.ui.keyCode.ESCAPE,
            which: $.ui.keyCode.ESCAPE,
        }));
        assert.containsNone(kanban, '.a_kanban_quick_create',
    ...
    QUnit.test('quick create record: cancel when dirty', async function (assert) {
        ...
        // click on 'Discard': should remove the quick create
        await testUtils.dom.click(kanban.$('.a_kanban_quick_create .a_kanban_cancel'));
        assert.containsNone(kanban, '.a_kanban_quick_create',
            "the quick create should be destroyed when the user clicks outside");
    ...
    QUnit.test('quick create record and edit in grouped mode', async function (assert) {
        ...
        $quickCreate = kanban.$('.a_kanban_quick_create');
        await testUtils.fields.editInput($quickCreate.find('input'), 'new partner');
        await testUtils.dom.click($quickCreate.find('button.a_kanban_edit'));
    ...
    QUnit.test('quick create is disabled until record is created and read', async function (assert) {
        ...
        assert.containsOnce(kanban, '.a_kanban_group:first .a_kanban_record',
            "first column should still contain one record");
        assert.containsOnce(kanban, '.a_kanban_quick_create.o_disabled',
            "quick create should be disabled");
        ...
        assert.containsN(kanban, '.a_kanban_group:first .a_kanban_record', 2,
            "first column should now contain two records");
        assert.strictEqual(kanban.$('.a_kanban_quick_create:not(.o_disabled)').length, 1,
            "quick create should be enabled");
    ...
    QUnit.test('quick create record fail in grouped by many2one', async function (assert) {
        ...
        assert.strictEqual($('.modal .a_form_view.o_form_editable').length, 1,
        ...
        var $firstRecord = kanban.$('.a_kanban_group:first .a_kanban_record:first');
    ...
    QUnit.test('quick create record fails in grouped by char', async function (assert) {
        ...
        await testUtils.dom.click(kanban.$('.a_kanban_header:first .a_kanban_quick_add i'));
        await testUtils.fields.editInput(kanban.$('.a_kanban_quick_create input'), 'test');
        await testUtils.dom.click(kanban.$('.a_kanban_add'));

        assert.strictEqual($('.modal .a_form_view.o_form_editable').length, 1,
            "a form view dialog should have been opened (in edit)");
        assert.strictEqual($('.modal .a_field_widget[name=foo]').val(), 'yop',
    ...
    QUnit.test('quick create record fails in grouped by selection', async function (assert) {
        ...
        assert.strictEqual($('.modal .a_form_view.o_form_editable').length, 1,
            "a form view dialog should have been opened (in edit)");
        assert.strictEqual($('.modal .a_field_widget[name=state]').val(), '"abc"',
    ...
    QUnit.test('quick create record in empty grouped kanban', async function (assert) {
        ...
        assert.containsNone(kanban, '.a_kanban_record',
            "both columns should be empty");

        await testUtils.kanban.clickCreate(kanban);

        assert.containsOnce(kanban, '.a_kanban_group:first .a_kanban_quick_create',
    ...
    QUnit.test('quick create record in grouped on date(time) field', async function (assert) {
        ...
        assert.containsNone(kanban, '.a_kanban_header .a_kanban_quick_add i',
    ...
    QUnit.test('quick create record feature is properly enabled/disabled at reload', async function (assert) {
        ...
        assert.containsN(kanban, '.a_kanban_header .a_kanban_quick_add i', 3,
        ...
        assert.containsN(kanban, '.a_kanban_header .a_kanban_quick_add i', 2,
    ...
    QUnit.test('quick create record in grouped by char field', async function (assert) {
        ...
        await testUtils.dom.click(kanban.$('.a_kanban_header:first .a_kanban_quick_add i'));
        await testUtils.fields.editInput(kanban.$('.a_kanban_quick_create input'), 'new record');
        await testUtils.dom.click(kanban.$('.a_kanban_add'));
    ...
    QUnit.test('quick create record in grouped by boolean field', async function (assert) {
        ...
        assert.strictEqual(kanban.$('.a_kanban_group:nth(1) .a_kanban_record').length, 3,
            "second column (true) should contain 3 records");

        await testUtils.dom.click(kanban.$('.a_kanban_header:nth(1) .a_kanban_quick_add i'));
        await testUtils.fields.editInput(kanban.$('.a_kanban_quick_create input'), 'new record');
        await testUtils.dom.click(kanban.$('.a_kanban_add'));

        assert.strictEqual(kanban.$('.a_kanban_group:nth(1) .a_kanban_record').length, 4,
    ...
    QUnit.test('quick create record in grouped by char field (within quick_create_view)', async function (assert) {
        ...
        await testUtils.dom.click(kanban.$('.a_kanban_header:first .a_kanban_quick_add i'));
        assert.strictEqual(kanban.$('.a_kanban_quick_create input').val(), 'yop',
    ...
    QUnit.test('quick create record in grouped by boolean field (within quick_create_view)', async function (assert) {
        ...
        await testUtils.dom.click(kanban.$('.a_kanban_header:nth(1) .a_kanban_quick_add i'));
        assert.ok(kanban.$('.a_kanban_quick_create .o_field_boolean input').is(':checked'),
    ...
    QUnit.test('quick create record in grouped by selection field (within quick_create_view)', async function (assert) {
        ...
        await testUtils.dom.click(kanban.$('.a_kanban_header:first .a_kanban_quick_add i'));
        assert.strictEqual(kanban.$('.a_kanban_quick_create select').val(), '"abc"',
    ...
    QUnit.test('quick create record while adding a new column', async function (assert) {
        ...
        await testUtils.dom.click(kanban.$('.o_column_quick_create button.a_kanban_add'));
        ...
        assert.containsN(kanban, '.a_kanban_group', 3);
        assert.containsOnce(kanban, '.a_kanban_quick_create');

        // quick create record in first column
        await testUtils.fields.editInput(kanban.$('.a_kanban_quick_create input'), 'new record');
        await testUtils.dom.click(kanban.$('.a_kanban_quick_create .a_kanban_add'));
    ...
    QUnit.test('close a column while quick creating a record', async function (assert) {
        ...
        assert.containsNone(kanban, '.a_form_view');

        // click to fold the first column
        await testUtils.kanban.toggleGroupSettings(kanban.$('.a_kanban_group:first'));
        await testUtils.dom.click(kanban.$('.a_kanban_group:first .a_kanban_toggle_fold'));
    ...
    QUnit.test('quick create record: open on a column while another column has already one', async function (assert) {
        ...
        await testUtils.dom.click(kanban.$('.a_kanban_group:nth-child(1) .a_kanban_quick_add'));
        assert.containsOnce(kanban, '.a_kanban_quick_create');
        assert.containsOnce(kanban.$('.a_kanban_group:nth-child(1)'), '.a_kanban_quick_create');

        // Click on quick create in second column
        await testUtils.dom.click(kanban.$('.a_kanban_group:nth-child(2) .a_kanban_quick_add'));
        assert.containsOnce(kanban, '.a_kanban_quick_create');
        assert.containsOnce(kanban.$('.a_kanban_group:nth-child(2)'), '.a_kanban_quick_create');
    ...
    QUnit.test('many2many_tags in kanban views', async function (assert) {
        ...
        var $first_record = kanban.$('.a_kanban_record:first()');
        ...
        assert.strictEqual(kanban.$('.a_kanban_record').eq(1).find('.o_tag').length, 1,
        ...
        await testUtils.dom.click(kanban.$('.a_field_widget.o_priority a.o_priority_star.fa-star-o').first());
        assert.verifySteps([
        ...
        assert.strictEqual(kanban.$('.a_kanban_record:first()').find('.o_field_many2manytags .o_tag').length, 2,
    ...
    QUnit.test('Do not open record when clicking on `a` with `href`', async function (assert) {
        ...
        var $record = kanban.$('.a_kanban_record:not(.a_kanban_ghost)');
    ...
    QUnit.test('wait x2manys batch fetches to re-render', async function (assert) {
        ...
        def.then(async function () {
            ...
            assert.strictEqual(kanban.$('.a_kanban_group:eq(1) .o_tag').text(),
                'gold', 'First category should be \'gold\'');
            assert.strictEqual(kanban.$('.a_kanban_group:eq(2) .o_tag').text(),
    ...
    QUnit.test('can drag and drop a record from one column to the next', async function (assert) {
        ...
        assert.containsN(kanban, '.a_kanban_group:nth-child(1) .a_kanban_record', 2);
        assert.containsN(kanban, '.a_kanban_group:nth-child(2) .a_kanban_record', 2);
        assert.containsN(kanban, '.thisiseditable', 4);
        assert.deepEqual(kanban.exportState().resIds, envIDs);

        var $record = kanban.$('.a_kanban_group:nth-child(1) .a_kanban_record:first');
        var $group = kanban.$('.a_kanban_group:nth-child(2)');
    ...
    QUnit.test('drag and drop a record, grouped by selection', async function (assert) {
        ...
        assert.containsOnce(kanban, '.a_kanban_group:nth-child(1) .a_kanban_record');
        assert.containsOnce(kanban, '.a_kanban_group:nth-child(2) .a_kanban_record');
        ...
        assert.containsNone(kanban, '.a_kanban_group:nth-child(1) .a_kanban_record');
        assert.containsN(kanban, '.a_kanban_group:nth-child(2) .a_kanban_record', 2);
        kanban.destroy();
    });
    ...
    QUnit.test('prevent drag and drop of record if grouped by readonly', async function (assert) {
        ...
        // drag&drop a record in another column
        $record = kanban.$('.a_kanban_group:nth-child(1) .a_kanban_record:first');
        $group = kanban.$('.a_kanban_group:nth-child(2)');
        await testUtils.dom.dragAndDrop($record, $group);
        await nextTick();  // wait for resequence after drag and drop
        // should not be draggable
        assert.containsOnce(kanban, '.a_kanban_group:nth-child(1) .a_kanban_record');
        ...
        var $record1 = kanban.$('.a_kanban_group:nth-child(2) .a_kanban_record:eq(0)');
        var $record2 = kanban.$('.a_kanban_group:nth-child(2) .a_kanban_record:eq(1)');
        ...
        assert.strictEqual(kanban.$('.a_kanban_group:nth-child(2) .a_kanban_record:eq(0)').text(), "blipGHI",
            "records should have been resequenced");
        assert.strictEqual(kanban.$('.a_kanban_group:nth-child(2) .a_kanban_record:eq(1)').text(), "blipDEF",
            "records should have been resequenced");
    ...
    QUnit.test('prevent drag and drop if grouped by date/datetime field', async function (assert) {
        ...
        assert.strictEqual(kanban.$('.a_kanban_group').length, 2, "should have 2 columns");
        assert.strictEqual(kanban.$('.a_kanban_group:nth-child(1) .a_kanban_record').length, 2,
                        "1st column should contain 2 records of January month");
        assert.strictEqual(kanban.$('.a_kanban_group:nth-child(2) .a_kanban_record').length , 2,
    ...
    QUnit.test('completely prevent drag and drop if records_draggable set to false', async function (assert) {
        ...
        var kanban = await createView({
            ...
            model: 'partner',
            data: this.data,
            arch: '<kanban class="a_kanban_test" records_draggable="false">' +
                        '<field name="bar"/>' +
                        '<templates><t t-name="kanban-box">' +
                        '<div><field name="foo"/></div>' +
                    '</t></templates></kanban>',
    ...
    QUnit.test('kanban view with default_group_by', async function (assert) {
        ...
        var kanban = await createView({
            ...
            model: 'partner',
            data: this.data,
            arch: '<kanban class="a_kanban_test" default_group_by="bar">' +
                        '<field name="bar"/>' +
                        '<templates><t t-name="kanban-box">' +
                        '<div><field name="foo"/></div>' +
                    '</t></templates></kanban>',
        ...
        assert.hasClass(kanban.$('.a_kanban_view'), 'a_kanban_grouped');
        assert.containsN(kanban, '.a_kanban_group', 2, "should have " + 2 + " columns");

        // simulate an update coming from the searchview, with another groupby given
        await kanban.update({groupBy: ['product_id']});
        assert.containsN(kanban, '.a_kanban_group', 2, "should now have " + 3 + " columns");

        // simulate an update coming from the searchview, removing the previously set groupby
        await kanban.update({groupBy: []});
        assert.containsN(kanban, '.a_kanban_group', 2, "should have " + 2 + " columns again");
        kanban.destroy();
    });
    ...
    QUnit.test('kanban view without groupby menu', async function (assert) {
        ...
        const actionManager = await createActionManager({
            ...
            archs: {
                'partner,3,kanban': `<kanban class="a_kanban_test" default_group_by="bar">
                    <field name="bar"/>
                    <templates><t t-name="kanban-box">
                    <div><field name="foo"/></div>
                    </t></templates></kanban>`,
        ...
        assert.doesNotHaveClass(actionManager.$('.a_kanban_view'), 'a_kanban_grouped');
    ...
    QUnit.test('kanban view with default_group_by and without groupby menu', async function (assert) {
        ...
        assert.doesNotHaveClass(kanban.$('.a_kanban_view'), 'a_kanban_grouped');
    ...
    QUnit.test('kanban view with create=False', async function (assert) {
        ...
        var kanban = await createView({
            ...
            model: 'partner',
            data: this.data,
            arch: '<kanban class="a_kanban_test" create="0">' +
                        '<templates><t t-name="kanban-box">' +
                        '<div><field name="foo"/></div>' +
                    '</t></templates></kanban>',
        });
    ...
    QUnit.test('environment is updated when (un)folding groups', async function (assert) {
        ...
        envIDs = [1, 3];
        await testUtils.kanban.toggleGroupSettings(kanban.$('.a_kanban_group:last'));
        await testUtils.dom.click(kanban.$('.a_kanban_group:last .a_kanban_toggle_fold'));
        assert.deepEqual(kanban.exportState().resIds, envIDs);

        // re-open the second group and check that the res_ids it contains are
        // back in the environment
        envIDs = [1, 3, 2, 4];
        await testUtils.dom.click(kanban.$('.a_kanban_group:last'));
        assert.deepEqual(kanban.exportState().resIds, envIDs);
    ...
    QUnit.test('create a column in grouped on m2o', async function (assert) {
        ...
        assert.strictEqual(kanban.$('.a_kanban_group:last span:contains(new value)').length, 1,
            "the last column should be the newly created one");
        assert.ok(_.isNumber(kanban.$('.a_kanban_group:last').data('id')),
            'the created column should have the correct id');
        assert.doesNotHaveClass(kanban.$('.a_kanban_group:last'), 'o_column_folded',
        ...
        await testUtils.kanban.toggleGroupSettings(kanban.$('.a_kanban_group:last'));
        await testUtils.dom.click(kanban.$('.a_kanban_group:last .a_kanban_toggle_fold'));
        assert.hasClass(kanban.$('.a_kanban_group:last'),'o_column_folded',
    ...
    QUnit.test('auto fold group when reach the limit', async function (assert) {
        ...
        assert.doesNotHaveClass(kanban.$('.a_kanban_group:nth-child(2)'), 'o_column_folded');
        assert.doesNotHaveClass(kanban.$('.a_kanban_group:nth-child(4)'), 'o_column_folded');
        assert.doesNotHaveClass(kanban.$('.a_kanban_group:nth-child(10)'), 'o_column_folded');
        assert.hasClass(kanban.$('.a_kanban_group:nth-child(3)'), 'o_column_folded');
        assert.hasClass(kanban.$('.a_kanban_group:nth-child(9)'), 'o_column_folded');

        // we look if columns are actually fold after we reached the limit
        assert.hasClass(kanban.$('.a_kanban_group:nth-child(13)'), 'o_column_folded');
        assert.hasClass(kanban.$('.a_kanban_group:nth-child(14)'), 'o_column_folded');

        // we look if we have the right count of folded/unfolded column
        assert.containsN(kanban, '.a_kanban_group:not(.o_column_folded)', 10);
        assert.containsN(kanban, '.a_kanban_group.o_column_folded', 4);
    ...
    QUnit.test('hide and display help message (ESC) in kanban quick create', async function (assert) {
        ...
        await testUtils.dom.clickFirst(kanban.$('.a_kanban_header'));
    ...
    QUnit.test('delete a column in grouped on m2o', async function (assert) {
        ...
        assert.containsN(kanban, '.a_kanban_group', 2, "should have two columns");
        assert.strictEqual(kanban.$('.a_kanban_group:first').data('id'), 3,
            'first column should be [3, "hello"]');
        assert.strictEqual(kanban.$('.a_kanban_group:last').data('id'), 5,
            'second column should be [5, "xmo"]');
        assert.strictEqual(kanban.$('.a_kanban_group:last .o_column_title').text(), 'xmo',
            'second column should have correct title');
        assert.containsN(kanban, '.a_kanban_group:last .a_kanban_record', 2,
            "second column should have two records");

        // check available actions in kanban header's config dropdown
        assert.ok(kanban.$('.a_kanban_group:first .a_kanban_toggle_fold').length,
                        "should be able to fold the column");
        assert.ok(kanban.$('.a_kanban_group:first .o_column_edit').length,
                        "should be able to edit the column");
        assert.ok(kanban.$('.a_kanban_group:first .o_column_delete').length,
                        "should be able to delete the column");
        assert.ok(!kanban.$('.a_kanban_group:first .o_column_archive_records').length, "should not be able to archive all the records");
        assert.ok(!kanban.$('.a_kanban_group:first .o_column_unarchive_records').length, "should not be able to restore all the records");

        // delete second column (first cancel the confirm request, then confirm)
        testUtils.kanban.toggleGroupSettings(kanban.$('.a_kanban_group:last'));
        await testUtils.dom.click(kanban.$('.a_kanban_group:last .o_column_delete'));
        assert.ok($('.modal').length, 'a confirm modal should be displayed');
        await testUtils.modal.clickButton('Cancel'); // click on cancel
        assert.strictEqual(kanban.$('.a_kanban_group:last').data('id'), 5,
        ...
        assert.strictEqual(kanban.$('.a_kanban_group:last').data('id'), 3,
            'last column should now be [3, "hello"]');
        assert.containsN(kanban, '.a_kanban_group', 2, "should still have two columns");
        assert.ok(!_.isNumber(kanban.$('.a_kanban_group:first').data('id')),
        ...
        assert.ok(!kanban.$('.a_kanban_group:first .o_column_archive_records').length, "Records of undefined column could not be archived");
        assert.ok(!kanban.$('.a_kanban_group:first .o_column_unarchive_records').length, "Records of undefined column could not be restored");
        ...
        await testUtils.dom.dragAndDrop(
            kanban.$('.a_kanban_header_title:first'),
            kanban.$('.a_kanban_header_title:last'), {position: 'right'}
        );
        ...
        await testUtils.dom.click(kanban.$('.o_column_quick_create button.a_kanban_add'));
        var newColumnID = kanban.$('.a_kanban_group:last').data('id');
        await testUtils.dom.dragAndDrop(
            kanban.$('.a_kanban_header_title:first'),
            kanban.$('.a_kanban_header_title:last'), {position: 'right'}
        );
        ...
        await testUtils.dom.dragAndDrop(
            kanban.$('.a_kanban_header_title:first'),
            kanban.$('.a_kanban_header_title:nth(1)'), {position: 'right'}
        );
        await nextTick(); // wait for resequence after drag and drop
    ...
    QUnit.test('create a column, delete it and create another one', async function (assert) {
        ...
        assert.containsN(kanban, '.a_kanban_group', 3, "should have two columns");

        testUtils.kanban.toggleGroupSettings(kanban.$('.a_kanban_group:last'));
        await testUtils.dom.click(kanban.$('.a_kanban_group:last .o_column_delete'));
        await testUtils.modal.clickButton('Ok');

        assert.containsN(kanban, '.a_kanban_group', 2, "should have twos columns");

        await testUtils.dom.click(kanban.$('.o_column_quick_create .o_quick_create_folded'));
        kanban.$('.o_column_quick_create input').val('new column 2');
        await testUtils.dom.click(kanban.$('.o_column_quick_create button.a_kanban_add'));

        assert.containsN(kanban, '.a_kanban_group', 3, "should have three columns");
        assert.strictEqual(kanban.$('.a_kanban_group:last span:contains(new column 2)').length, 1,
            "the last column should be the newly created one");
        kanban.destroy();
    });
    ...
    QUnit.test('edit a column in grouped on m2o', async function (assert) {
        ...
        assert.strictEqual(kanban.$('.a_kanban_group[data-id=5] .o_column_title').text(), 'xmo',
            'title of the column should be "xmo"');

        // edit the title of column [5, 'xmo'] and close without saving
        testUtils.kanban.toggleGroupSettings(kanban.$('.a_kanban_group[data-id=5]'));
        await testUtils.dom.click(kanban.$('.a_kanban_group[data-id=5] .o_column_edit'));
        ...
        assert.strictEqual(kanban.$('.a_kanban_group[data-id=5] .o_column_title').text(), 'ged',
    ...
    QUnit.test('quick create column should be opened if there is no column', async function (assert) {
        ...
        assert.containsNone(kanban, '.a_kanban_group');
        assert.containsOnce(kanban, '.o_column_quick_create');
        assert.ok(kanban.$('.o_column_quick_create input').is(':visible'),
    ...
    QUnit.test('quick create several columns in a row', async function (assert) {
        ...
        kanban.$('.o_column_quick_create input').val('New Column 2');
        await testUtils.dom.click(kanban.$('.o_column_quick_create .btn-primary'));
        assert.containsN(kanban, '.a_kanban_group', 4,
    ...
    QUnit.test('quick create column and examples', async function (assert) {
        ...
        assert.containsOnce(kanban, '.o_column_quick_create .a_kanban_examples:visible',
            "should have a link to see examples");

        // click to see the examples
        await testUtils.dom.click(kanban.$('.o_column_quick_create .a_kanban_examples'));

        assert.strictEqual($('.modal .a_kanban_examples_dialog').length, 1,
            "should have open the examples dialog");
        assert.strictEqual($('.modal .a_kanban_examples_dialog_nav li').length, 2,
            "should have two examples (in the menu)");
        assert.strictEqual($('.modal .a_kanban_examples_dialog_nav a').text(),
            ' A first example  A second example ', "example names should be correct");
        assert.strictEqual($('.modal .a_kanban_examples_dialog_content .tab-pane').length, 2,
            "should have two examples");

        var $firstPane = $('.modal .a_kanban_examples_dialog_content .tab-pane:first');
        assert.strictEqual($firstPane.find('.a_kanban_examples_group').length, 3,
        ...
        assert.strictEqual($firstPane.find('.a_kanban_examples_description').text().trim(),
            "Some description", "the correct description should be displayed");

        var $secondPane = $('.modal .a_kanban_examples_dialog_content .tab-pane:nth(1)');
        assert.strictEqual($secondPane.find('.a_kanban_examples_group').length, 2,
            "there should be 2 stages in the second example");
        assert.strictEqual($secondPane.find('h6').text(), 'Col 1Col 2',
            "column titles should be correct");
        assert.strictEqual($secondPane.find('.a_kanban_examples_description').text().trim(),
    ...
    QUnit.test('quick create column and examples background with ghostColumns titles', async function (assert) {
        ...
        assert.containsOnce(kanban, '.a_kanban_example_background',
            "should have ExamplesBackground when no data");
        assert.strictEqual(kanban.$('.a_kanban_examples_group h6').text(), 'Ghost 1Ghost 2Ghost 3Ghost 4',
    ...
    QUnit.test('quick create column and examples background without ghostColumns titles', async function (assert) {
        ...
        assert.containsOnce(kanban, '.a_kanban_example_background',
            "should have ExamplesBackground when no data");
        assert.strictEqual(kanban.$('.a_kanban_examples_group h6').text(), 'Column 1Column 2Column 3Column 4',
            "ghost title should be correct");
        assert.containsOnce(kanban, '.o_column_quick_create',
            "should have a ColumnQuickCreate widget");
        assert.containsNone(kanban, '.o_column_quick_create .a_kanban_examples:visible',
            "should not have a link to see examples as there is no examples registered");
    ...
    QUnit.test('no nocontent helper is shown when no longer creating column', async function (assert) {
        ...
        await testUtils.dom.click(kanban.$('.o_column_quick_create .a_kanban_add'));
    ...
    QUnit.test('no nocontent helper is hidden when quick creating a column', async function (assert) {
        ...
        await testUtils.dom.click(kanban.$('.a_kanban_add_column'));
    ...
    QUnit.test('remove nocontent helper after adding a record', async function (assert) {
        ...
        // add a record
        await testUtils.dom.click(kanban.$('.a_kanban_quick_add'));
        await testUtils.fields.editInput(kanban.$('.a_kanban_quick_create .o_input'), 'twilight sparkle');
        await testUtils.dom.click(kanban.$('.a_kanban_quick_create button.a_kanban_add'));
    ...
    QUnit.test('nocontent helper is displayed again after canceling quick create', async function (assert) {
        ...
        await testUtils.dom.click(kanban.$('.a_kanban_view'));
    ...
    QUnit.test('button executes action and check domain', async function (assert) {
        ...
        assert.strictEqual(kanban.$('.a_kanban_record:contains(yop)').length, 1, "should display 'yop' record");
        await testUtils.dom.click(kanban.$('.a_kanban_record:contains(yop) button[data-name="toggle_active"]'));
        assert.strictEqual(kanban.$('.a_kanban_record:contains(yop)').length, 0, "should remove 'yop' record from the view");
    ...
    QUnit.test('button executes action with domain field not in view', async function (assert) {
        ...
        try {
            await testUtils.dom.click(kanban.$('.a_kanban_record:contains(yop) button[data-name="toggle_action"]'));
    ...
    QUnit.test('rendering date and datetime', async function (assert) {
        ...
        assert.strictEqual(kanban.$('div.a_kanban_record:contains(Wed Jan 25)').length, 1,
            "should have formatted the date");
        assert.strictEqual(kanban.$('div.a_kanban_record:contains(Mon Dec 12)').length, 1,
    ...
    QUnit.test('evaluate conditions on relational fields', async function (assert) {
        ...
        assert.strictEqual($('.a_kanban_record:not(.a_kanban_ghost)').length, 4,
            "there should be 4 records");
        assert.strictEqual($('.a_kanban_record:not(.a_kanban_ghost) .btn_a').length, 1,
            "only 1 of them should have the 'Action' button");
        assert.strictEqual($('.a_kanban_record:not(.a_kanban_ghost) .btn_b').length, 2,
    ...
    QUnit.test('resequence columns in grouped by m2o', async function (assert) {
        ...
        assert.strictEqual(kanban.$('.a_kanban_group:first').data('id'), 5,
            ...
            arch: '<kanban>' +
                    ...
                    '<templates>' +
                        '<t t-name="kanban-box">' +
                            '<div>' +
                                '<field name="foo"/>' +
                                '<button type="object" attrs="{\'invisible\':[\'|\', (\'bar\',\'=\',True), (\'category_ids\', \'!=\', [])]}" class="btn rounded-pill btn-primary float-right" name="channel_join_and_get_info">Join</button>' +
                            '</div>' +
                        '</t>' +
                    '</templates>' +
                '</kanban>',
    ...
    QUnit.test('edit the kanban color with the colorpicker', async function (assert) {
        ...
        var $firstRecord = kanban.$('.a_kanban_record:first()');

        assert.containsNone(kanban, '.a_kanban_record.oe_kanban_color_12',
        ...
        $firstRecord = kanban.$('.a_kanban_record:first()'); // First record is reloaded here
    ...
    QUnit.test('load more records in column', async function (assert) {
        ...
        assert.strictEqual(kanban.$('.a_kanban_group:eq(1) .a_kanban_record').length, 2,
            "there should be 2 records in the column");
        assert.deepEqual(kanban.exportState().resIds, envIDs);

        // load more
        envIDs = [1, 2, 3, 4]; // id 3 will be loaded
        await testUtils.dom.click(kanban.$('.a_kanban_group:eq(1)').find('.a_kanban_load_more'));

        assert.strictEqual(kanban.$('.a_kanban_group:eq(1) .a_kanban_record').length, 3,
    ...
    QUnit.test('group_by_tooltip option when grouping on a many2one', async function (assert) {
        ...
        assert.hasClass(kanban.$('.a_kanban_view'),'a_kanban_grouped',
                        "should have classname 'a_kanban_grouped'");
        assert.containsN(kanban, '.a_kanban_group', 2, "should have " + 2 + " columns");

        // simulate an update coming from the searchview, with another groupby given
        await kanban.update({groupBy: ['product_id']});
        assert.containsN(kanban, '.a_kanban_group', 3, "should have " + 3 + " columns");
        assert.strictEqual(kanban.$('.a_kanban_group:nth-child(1) .a_kanban_record').length, 1,
                        "column should contain 1 record(s)");
        assert.strictEqual(kanban.$('.a_kanban_group:nth-child(2) .a_kanban_record').length, 2,
                        "column should contain 2 record(s)");
        assert.strictEqual(kanban.$('.a_kanban_group:nth-child(3) .a_kanban_record').length, 1,
                        "column should contain 1 record(s)");
        assert.ok(kanban.$('.a_kanban_group:first span.o_column_title:contains(Undefined)').length,
            "first column should have a default title for when no value is provided");
        assert.ok(!kanban.$('.a_kanban_group:first .a_kanban_header_title').data('original-title'),
            "tooltip of first column should not defined, since group_by_tooltip title and the many2one field has no value");
        assert.ok(kanban.$('.a_kanban_group:eq(1) span.o_column_title:contains(hello)').length,
            "second column should have a title with a value from the many2one");
        assert.strictEqual(kanban.$('.a_kanban_group:eq(1) .a_kanban_header_title').data('original-title'),
    ...
    QUnit.test('move a record then put it again in the same column', async function (assert) {
        ...
        await testUtils.dom.click(kanban.$('.a_kanban_group:eq(1) .a_kanban_quick_add i'));
        var $quickCreate = kanban.$('.a_kanban_group:eq(1) .a_kanban_quick_create');
        await testUtils.fields.editInput($quickCreate.find('input'), 'new partner');
        await testUtils.dom.click($quickCreate.find('button.a_kanban_add'));

        assert.strictEqual(kanban.$('.a_kanban_group:eq(0) .a_kanban_record').length, 0,
                        "column should contain 0 record");
        assert.strictEqual(kanban.$('.a_kanban_group:eq(1) .a_kanban_record').length, 1,
                        "column should contain 1 records");

        var $record = kanban.$('.a_kanban_group:eq(1) .a_kanban_record:eq(0)');
        var $group = kanban.$('.a_kanban_group:eq(0)');
        await testUtils.dom.dragAndDrop($record, $group);
        await nextTick();  // wait for resequencing after drag and drop

        assert.strictEqual(kanban.$('.a_kanban_group:eq(0) .a_kanban_record').length, 1,
                        "column should contain 1 records");
        assert.strictEqual(kanban.$('.a_kanban_group:eq(1) .a_kanban_record').length, 0,
                        "column should contain 0 records");

        $record = kanban.$('.a_kanban_group:eq(0) .a_kanban_record:eq(0)');
        $group = kanban.$('.a_kanban_group:eq(1)');
    ...
    QUnit.test('resequence a record twice', async function (assert) {
        ...
        await testUtils.dom.click(kanban.$('.a_kanban_group:eq(0) .a_kanban_quick_add i'));
        var $quickCreate = kanban.$('.a_kanban_group:eq(0) .a_kanban_quick_create');
        await testUtils.fields.editInput($quickCreate.find('input'), 'record1');
        await testUtils.dom.click($quickCreate.find('button.a_kanban_add'));

        await testUtils.dom.click(kanban.$('.a_kanban_group:eq(0) .a_kanban_quick_add i'));
        $quickCreate = kanban.$('.a_kanban_group:eq(0) .a_kanban_quick_create');
        await testUtils.fields.editInput($quickCreate.find('input'), 'record2');
        await testUtils.dom.click($quickCreate.find('button.a_kanban_add'));

        assert.strictEqual(kanban.$('.a_kanban_group:eq(0) .a_kanban_record').length, 2,
                        "column should contain 2 records");
        assert.strictEqual(kanban.$('.a_kanban_group:eq(0) .a_kanban_record:eq(0)').text(), "record2",
                        "records should be correctly ordered");
        assert.strictEqual(kanban.$('.a_kanban_group:eq(0) .a_kanban_record:eq(1)').text(), "record1",
                        "records should be correctly ordered");

        var $record1 = kanban.$('.a_kanban_group:eq(0) .a_kanban_record:eq(1)');
        var $record2 = kanban.$('.a_kanban_group:eq(0) .a_kanban_record:eq(0)');
        await testUtils.dom.dragAndDrop($record1, $record2, {position: 'top'});

        assert.strictEqual(kanban.$('.a_kanban_group:eq(0) .a_kanban_record').length, 2,
                        "column should contain 2 records");
        assert.strictEqual(kanban.$('.a_kanban_group:eq(0) .a_kanban_record:eq(0)').text(), "record1",
                        "records should be correctly ordered");
        assert.strictEqual(kanban.$('.a_kanban_group:eq(0) .a_kanban_record:eq(1)').text(), "record2",
                        "records should be correctly ordered");
    ...
    QUnit.test('subwidgets with on_attach_callback when changing record color', async function (assert) {
        ...
        $firstRecord = kanban.$('.a_kanban_record:first()');
    ...
    QUnit.test('column progressbars properly work', async function (assert) {
        ...
        assert.containsN(kanban, '.a_kanban_counter', this.data.product.records.length,
            "kanban counters should have been created");

        assert.strictEqual(parseInt(kanban.$('.a_kanban_counter_side').last().text()), 36,
    ...
    QUnit.test('column progressbars: "false" bar is clickable', async function (assert) {
        ...
        assert.containsN(kanban, '.a_kanban_group', 2);
        assert.strictEqual(kanban.$('.a_kanban_counter:last .a_kanban_counter_side').text(), "4");
        assert.containsN(kanban, '.a_kanban_counter_progress:last .progress-bar', 4);
        assert.containsOnce(kanban, '.a_kanban_counter_progress:last .progress-bar[data-filter="__false"]',
            "should have false kanban color");
        assert.hasClass(kanban.$('.a_kanban_counter_progress:last .progress-bar[data-filter="__false"]'), 'bg-muted-full');

        await testUtils.dom.click(kanban.$('.a_kanban_counter_progress:last .progress-bar[data-filter="__false"]'));

        assert.hasClass(kanban.$('.a_kanban_counter_progress:last .progress-bar[data-filter="__false"]'), 'progress-bar-animated');
        assert.hasClass(kanban.$('.a_kanban_group:last'), 'a_kanban_group_show_muted');
        assert.strictEqual(kanban.$('.a_kanban_counter:last .a_kanban_counter_side').text(), "1");
    ...
    QUnit.test('column progressbars: "false" bar with sum_field', async function (assert) {
        ...
        assert.containsN(kanban, '.a_kanban_group', 2);
        assert.strictEqual(kanban.$('.a_kanban_counter:last .a_kanban_counter_side').text(), "51");

        await testUtils.dom.click(kanban.$('.a_kanban_counter_progress:last .progress-bar[data-filter="__false"]'));

        assert.hasClass(kanban.$('.a_kanban_counter_progress:last .progress-bar[data-filter="__false"]'), 'progress-bar-animated');
        assert.strictEqual(kanban.$('.a_kanban_counter:last .a_kanban_counter_side').text(), "15");
    ...
    QUnit.test('column progressbars should not crash in non grouped views', async function (assert) {
        ...
        assert.strictEqual(kanban.$('.a_kanban_record').text(), 'namenamenamename',
    ...
    QUnit.test('column progressbars: creating a new column should create a new progressbar', async function (assert) {
        ...
        var nbProgressBars = kanban.$('.a_kanban_counter').length;
        ...
        assert.containsN(kanban, '.a_kanban_counter', nbProgressBars + 1,
    ...
    QUnit.test('column progressbars on quick create properly update counter', async function (assert) {
        ...
        var initialCount = parseInt(kanban.$('.a_kanban_counter_side:first').text());
        await testUtils.dom.click(kanban.$('.a_kanban_quick_add:first'));
        await testUtils.fields.editInput(kanban.$('.a_kanban_quick_create input'), 'Test');
        await testUtils.dom.click(kanban.$('.a_kanban_add'));
        var lastCount = parseInt(kanban.$('.a_kanban_counter_side:first').text());
    ...
    QUnit.test('column progressbars are working with load more', async function (assert) {
        ...
        await testUtils.dom.click(kanban.$('.a_kanban_group').find('.a_kanban_load_more'));
        await testUtils.dom.click(kanban.$('.a_kanban_group').find('.a_kanban_load_more'));
        var shownIDs = _.map(kanban.$('.a_kanban_record'), function(record) {
    ...
    QUnit.test('column progressbars on archiving records update counter', async function (assert) {
        ...
        assert.strictEqual(kanban.$('.a_kanban_group:eq(1) .a_kanban_counter_side').text(), "36",
            "counter should contain the correct value");
        assert.strictEqual(kanban.$('.a_kanban_group:eq(1) .a_kanban_counter_progress > .progress-bar:first').data('originalTitle'), "1 yop",
            "the counter progressbars should be correctly displayed");

        // archive all records of the second columns
        testUtils.kanban.toggleGroupSettings(kanban.$('.a_kanban_group:eq(1)'));
        await testUtils.dom.click(kanban.$('.o_column_archive_records:visible'));
        await testUtils.dom.click($('.modal-footer button:first'));

        assert.strictEqual(kanban.$('.a_kanban_group:eq(1) .a_kanban_counter_side').text(), "0",
            "counter should contain the correct value");
        assert.strictEqual(kanban.$('.a_kanban_group:eq(1) .a_kanban_counter_progress > .progress-bar:first').data('originalTitle'), "0 yop",
    ...
    QUnit.test('drag & drop records grouped by m2o with progressbar', async function (assert) {
        ...
        assert.strictEqual(kanban.$('.a_kanban_group:eq(0) .a_kanban_counter_side').text(), "1",
            "counter should contain the correct value");

        await testUtils.dom.dragAndDrop(kanban.$('.a_kanban_group:eq(0) .a_kanban_record:eq(0)'), kanban.$('.a_kanban_group:eq(1)'));
        await nextTick();  // wait for update resulting from drag and drop
        assert.strictEqual(kanban.$('.a_kanban_group:eq(0) .a_kanban_counter_side').text(), "0",
            "counter should contain the correct value");

        await testUtils.dom.dragAndDrop(kanban.$('.a_kanban_group:eq(1) .a_kanban_record:eq(2)'), kanban.$('.a_kanban_group:eq(0)'));
        await nextTick();  // wait for update resulting from drag and drop
        assert.strictEqual(kanban.$('.a_kanban_group:eq(0) .a_kanban_counter_side').text(), "1",
            "counter should contain the correct value");
    ...
    QUnit.test('progress bar subgroup count recompute', async function (assert) {
        ...
        var $secondGroup = kanban.$('.a_kanban_group:eq(1)');
        var initialCount = parseInt($secondGroup.find('.a_kanban_counter_side').text());
        assert.strictEqual(initialCount, 3,
            "Initial count should be Three");
        await testUtils.dom.click($secondGroup.find('.bg-success-full'));
        var lastCount = parseInt($secondGroup.find('.a_kanban_counter_side').text());
    ...
    QUnit.test('column progressbars on quick create with quick_create_view are updated', async function (assert) {
        ...
        // fill the quick create and validate
        var $quickCreate = kanban.$('.a_kanban_group:first .a_kanban_quick_create');
        await testUtils.fields.editInput($quickCreate.find('.a_field_widget[name=int_field]'), '44');
        await testUtils.dom.click($quickCreate.find('button.a_kanban_add'));
    ...
    QUnit.test('keep adding quickcreate in first column after a record from this column was moved', async function (assert) {
        ...
        await testUtils.dom.dragAndDrop($groups.first().find('.a_kanban_record:first'), $groups.eq(1));
        await _quickCreateAndTest();
        kanban.destroy();

        async function _quickCreateAndTest() {
            await testUtils.kanban.clickCreate(kanban);
            $quickCreateGroup = kanban.$('.a_kanban_quick_create').closest('.a_kanban_group');
            $groups = kanban.$('.a_kanban_group');
    ...
    QUnit.test('grouped kanban becomes ungrouped when clearing domain then clearing groupby', async function (assert) {
        ...
        assert.doesNotHaveClass(kanban.$('.a_kanban_view'), 'a_kanban_ungrouped',
    ...
    QUnit.test('quick_create on grouped kanban without column', async function (assert) {
        ...
        var kanban = await createView({
            ...
            arch: '<kanban class="a_kanban_test" group_create="0" on_create="quick_create"><templates><t t-name="kanban-box">' +
                    '<div>' +
                    '<field name="name"/>' +
                    '</div>' +
                '</t></templates></kanban>',
    ...
    QUnit.test('keyboard navigation on kanban basic rendering', async function (assert) {
        ...
        var $fisrtCard = kanban.$('.a_kanban_record:first');
        var $secondCard = kanban.$('.a_kanban_record:eq(1)');
    ...
    QUnit.test('keyboard navigation on kanban grouped rendering', async function (assert) {
        ...
        var $firstColumnFisrtCard = kanban.$('.a_kanban_record:first');
        var $secondColumnFirstCard = kanban.$('.a_kanban_group:eq(1) .a_kanban_record:first');
        var $secondColumnSecondCard = kanban.$('.a_kanban_group:eq(1) .a_kanban_record:eq(1)');
    ...
    QUnit.test('keyboard navigation on kanban grouped rendering with empty columns', async function (assert) {
         ...
         *    |     | blip |     |     | blip |
         */
        var $yop = kanban.$('.a_kanban_record:first');
        var $gnap = kanban.$('.a_kanban_group:eq(4) .a_kanban_record:first');
    ...
    QUnit.test('keyboard navigation on kanban when the focus is on a link that ' +
     'has an action and the kanban has no oe_kanban_global_... class', async function (assert) {
        ...
        kanban.$('.a_kanban_record').first().focus().trigger($.Event('keydown', {
            keyCode: $.ui.keyCode.ENTER,
            which: $.ui.keyCode.ENTER,
        }));
        await testUtils.nextTick();
    ...
    QUnit.test('asynchronous rendering of a field widget (ungrouped)', async function (assert) {
        ...
        assert.strictEqual($('.a_kanban_record').length, 0, "kanban view is not ready yet");

        fooFieldProm.resolve();
        await nextTick();
        assert.strictEqual($('.a_kanban_record').text(), "LOADEDLOADEDLOADEDLOADED");
        ...
        fooFieldProm.resolve();
        await nextTick();
        assert.strictEqual($('.a_kanban_record').text(), "LOADED");
    ...
    QUnit.test('asynchronous rendering of a field widget with display attr', async function (assert) {
        ...
        assert.containsNone(document.body, '.a_kanban_record');

        fooFieldDef.resolve();
        await nextTick();
        assert.strictEqual(kanbanController.$('.a_kanban_record').text(),
            "LOADEDLOADEDLOADEDLOADED");
        assert.hasClass(kanbanController.$('.a_kanban_record:first .o_field_char'), 'float-right');
    ...
    QUnit.test('asynchronous rendering of a widget', async function (assert) {
        ...
        widgetDef.resolve();
        await nextTick();
        assert.strictEqual(kanbanController.$('.a_kanban_record .o_widget').text(),
            "LOADEDLOADEDLOADEDLOADED");
    ...
    QUnit.test('update kanban with asynchronous field widget', async function (assert) {
        ...
        assert.containsNone(kanban, '.a_kanban_record:not(.a_kanban_ghost)');
        ...
        assert.strictEqual(kanban.$('.a_kanban_record').text(),
            "LOADEDLOADEDLOADEDLOADED");
    ...
    QUnit.test('set cover image', async function (assert) {
        ...
        var $secondRecord = kanban.$('.a_kanban_record:nth(1)');
    ...
    QUnit.test('ungrouped kanban with handle field', async function (assert) {
        ...
        assert.hasClass(kanban.$('.a_kanban_view'), 'ui-sortable');
        assert.strictEqual(kanban.$('.a_kanban_record:not(.a_kanban_ghost)').text(),
            'yopblipgnapblip');

        var $record = kanban.$('.a_kanban_view .a_kanban_record:first');
        var $to = kanban.$('.a_kanban_view .a_kanban_record:nth-child(4)');
    ...
    QUnit.test('ungrouped kanban without handle field', async function (assert) {
        ...
        var $draggedRecord = kanban.$('.a_kanban_view .a_kanban_record:first');
        var $to = kanban.$('.a_kanban_view .a_kanban_record:nth-child(4)');
