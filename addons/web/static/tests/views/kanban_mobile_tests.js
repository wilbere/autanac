autanac.define('web.kanban_mobile_tests', function (require) {
"use strict";
...
QUnit.module('Views', {
    ...
    QUnit.test('mobile grouped rendering', async function (assert) {
        ...
        var kanban = await createView({
            ...
            model: 'partner',
            data: this.data,
            arch: '<kanban class="a_kanban_test a_kanban_small_column" on_create="quick_create">' +
        ...
        assert.hasClass(kanban.$('.a_kanban_mobile_tab:first'), 'o_current',
            "first tab is the active tab with class 'o_current'");
        assert.hasClass(kanban.$('.a_kanban_group:first'),'o_current',
            "first column is the active column with class 'o_current'");
        assert.containsN(kanban, '.a_kanban_group:first > div.a_kanban_record', 2,
            "there are 2 records in active tab");
        assert.strictEqual(kanban.$('.a_kanban_group:nth(1) > div.a_kanban_record').length, 0,
        ...
        assert.hasClass(kanban.$('.a_kanban_group:nth(0) > div:nth(1)'),'a_kanban_quick_create',
            "clicking on create should open the quick_create in the first column");

        // move to second column
        await testUtils.dom.click(kanban.$('.a_kanban_mobile_tab:nth(1)'));
        assert.hasClass(kanban.$('.a_kanban_mobile_tab:nth(1)'),'o_current',
            "second tab is now active with class 'o_current'");
        assert.hasClass(kanban.$('.a_kanban_group:nth(1)'),'o_current',
            "second column is now active with class 'o_current'");
        assert.strictEqual(kanban.$('.a_kanban_group:nth(1) > div.a_kanban_record').length, 2,
            "the 2 records of the second group have now been loaded");

        // quick create in second column
        await testUtils.dom.click(kanban.$buttons.find('.o-kanban-button-new'));
        assert.hasClass(kanban.$('.a_kanban_group:nth(1) >  div:nth(1)'),'a_kanban_quick_create',
            "clicking on create should open the quick_create in the second column");

        // kanban column should match kanban mobile tabs
        var column_ids = kanban.$('.a_kanban_group').map(function () {
            return $(this).data('id');
        }).get();
        var tab_ids = kanban.$('.a_kanban_mobile_tab').map(function () {
            return $(this).data('id');
        }).get();
        ...
        assert.containsN(kanban, '.a_kanban_mobile_tabs.justify-content-between', 1,
            "should have justify-content-between class");
        assert.hasClass(kanban.$('.a_kanban_mobile_tabs'), 'justify-content-between',
    ...
    QUnit.test('mobile grouped rendering in rtl direction', async function (assert) {
        ...
        assert.strictEqual(kanban.$('.a_kanban_group:first')[0].style.right, '0%',
            "first tab should have 50% right");
        assert.strictEqual(kanban.$('.a_kanban_group:nth(1)')[0].style.right, '100%',
    ...
    QUnit.test('mobile grouped with undefined column', async function (assert) {
        ...
        var kanban = await createView({
            ...
            model: 'partner',
            data: this.data,
            arch: '<kanban class="a_kanban_test a_kanban_small_column">' +
                    '<templates><t t-name="kanban-box">' +
                        '<div><field name="foo"/></div>' +
                    '</t></templates>' +
                '</kanban>',
        ...
        assert.containsOnce(kanban, '.a_kanban_columns_content .a_kanban_group:first-child[data-id^="partner_"]',
    ...
    QUnit.test('mobile grouped on many2one rendering', async function (assert) {
        ...
        // basic rendering tests
        assert.containsN(kanban, '.a_kanban_group', 4, "should have 4 columns");
        assert.containsN(kanban, '.a_kanban_group[data-id^="partner_"]', 4,
    ...
    QUnit.test('mobile quick create column view rendering', async function (assert) {
        ...
        // basic rendering tests
        assert.containsN(kanban, '.a_kanban_group', 2, "should have 2 columns");
        assert.hasClass(kanban.$('.a_kanban_view > .a_kanban_mobile_tabs_container > .a_kanban_mobile_tabs > div:last'), 'a_kanban_mobile_add_column',
        ...
        assert.hasClass(kanban.$('.a_kanban_group[data-id="3"].o_current > div:nth(1)'), 'a_kanban_quick_create',
        ...
        await testUtils.dom.click(kanban.$('.a_kanban_mobile_tab:last'));
        await testUtils.dom.click(kanban.$('.o-kanban-button-new'));
        assert.hasClass(kanban.$('.a_kanban_group[data-id="5"].o_current > div:nth(1)'), 'a_kanban_quick_create',
            "should open record quick create when clicking on create button in new column quick create");

        // new quick create column
        await testUtils.dom.click(kanban.$('.a_kanban_mobile_add_column'));
        ...
        assert.containsNone(kanban, '.a_kanban_examples', "Should not have See Examples link in mobile");
        ...
        assert.strictEqual(kanban.$('.a_kanban_group:last span:contains(msh)').length, 1,
    ...
    QUnit.test('mobile no quick create column when grouping on non m2o field', async function (assert) {
        ...
        assert.containsNone(kanban, '.a_kanban_mobile_add_column', "should not have the add column button");
